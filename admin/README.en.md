...
| 📊 Data reports | Summary/daily/CSV export | Redis 5-min cache, date span ≤90 days |
...
Listens on `http://0.0.0.0:8789` by default (can be changed via `APP_PORT` in `.env`; must stay consistent with `APP_URL`).
...
- **API paths**: `GET /admin/v1/user/{hashid}` — the `{id}` in the path is a hashid string
- **Database storage**: raw BIGINT value generated by snowflake

### API Versioning

API versions are placed in the URL path — public endpoints use `/api/v1/*`, admin endpoints use `/admin/v1/*` (default `v1`); no request header is used:

- Unsupported versions return `400 Bad Request`
- To add a new version, just create an `app/api/{version}/controller/` directory and register the new route group (e.g. `/api/v2`)
...
Cors (CORS preprocessing + response headers)
  ...
  → AdminAuth (JWT authentication + blacklist, /admin/v1 route group)
  → AdminPermission (RBAC authorization, /admin/v1 route group)
...
1. The client requests `POST /api/v1/captcha/generate` to get the captcha image (base64 PNG) and the list of target words
...
On logout the Token is added to the Redis blacklist and cannot be reused within its validity period. POST /admin/v1/profile/logout
...
> All `/api/v1/*` and `/admin/v1/*` endpoints carry their version number in the URL path; no version request header is used.
...
| `POST` | `/api/v1/captcha/verify` | Verify click captcha |
| `POST` | `/api/v1/auth/login` | Login (captcha required) |
| `POST` | `/api/v1/auth/register` | Register (captcha required) |
...
| Method | Path | Description |
|-----|------|------|
| `GET` | `/admin/v1/dashboard` | Dashboard data (Redis cache for 5 minutes) |
| `GET` | `/admin/v1/user` | User list (pagination + search) |
| `POST` | `/admin/v1/user` | Create user |
| `GET` | `/admin/v1/user/{id}` | User detail |
| `PUT` | `/admin/v1/user/{id}` | Update user |
| `DELETE` | `/admin/v1/user/{id}` | Delete user (soft delete, password confirmation required) |
| `POST` | `/admin/v1/user/batch/destroy` | Batch delete users (password confirmation required) |
| `POST` | `/admin/v1/user/batch/status` | Batch enable/disable users |
| `GET` | `/admin/v1/role` | Role list |
| `POST` | `/admin/v1/role` | Create role |
| `PUT` | `/admin/v1/role/{id}` | Update role |
| `DELETE` | `/admin/v1/role/{id}` | Delete role (password confirmation required) |
| `GET` | `/admin/v1/permission` | Permission tree |
| `POST` | `/admin/v1/permission` | Create permission |
| `PUT` | `/admin/v1/permission/{id}` | Update permission |
| `DELETE` | `/admin/v1/permission/{id}` | Delete permission (cascades to child permissions, password confirmation required) |
| `GET` | `/admin/v1/config` | System config list |
...
| `POST` | `/admin/v1/export/excel` | Export Excel |
| `POST` | `/admin/v1/export/pdf` | Export PDF |
| `POST` | `/admin/v1/import/users` | Import users from Excel |
| `POST` | `/admin/v1/upload` | File upload (images/documents, max 10MB) |
