# Plateforme mondiale d'agrégation de jeux — Plan d'implémentation (version de base)
<!-- lang-nav -->

Languages: [中文](2026-05-22-game-platform-plan.md) · [English](2026-05-22-game-platform-plan.en.md) · [한국어](2026-05-22-game-platform-plan.ko.md) · [Русский](2026-05-22-game-platform-plan.ru.md) · [Deutsch](2026-05-22-game-platform-plan.de.md) · **Français** · [Español](2026-05-22-game-platform-plan.es.md) · [Português](2026-05-22-game-platform-plan.pt.md) · [हिन्दी](2026-05-22-game-platform-plan.hi.md) · [العربية](2026-05-22-game-platform-plan.ar.md) · [বাংলা](2026-05-22-game-platform-plan.bn.md) · [Bahasa Indonesia](2026-05-22-game-platform-plan.id.md) · [日本語](2026-05-22-game-platform-plan.ja.md)


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Objectif :** boucler le cycle complet : inscription → dépôt → conversion → pièces de jeu → retrait → vérification en back-office

**Architecture :** admin/ (back-office) et service/ (côté C) sont deux instances webman v2 indépendantes, partageant les Modèles et Services via common/ (autoload PSR-4). Les migrations de base de données sont centralisées dans admin/database/migrations/. Frontend : extension Flutter PC du back-office + nouvelle plateforme PC côté C.

**Pile technique :** PHP 8.3+, webman v2, MySQL 8.0, Redis, composants de la série erikwang2013/*
...
- `common/model/` — Modèles Eloquent, en correspondance directe avec les tables, définissant casts/relations
- `service/app/api/v1/controller/` — API côté C, traite les requêtes des utilisateurs, appelle les Modèles
- `admin/app/admin/controller/` — API du back-office, opérations administrateur
- `common/middleware/UserAuth.php` — middleware de validation JWT côté C, injecte userId

---

## Phase 1 : Mise en place de l'infrastructure

### Task 1: Créer common/composer.json et définir l'espace de noms
...
### Task 2: Enregistrer common/ dans admin et créer la configuration d'autoload de service/
...
### Task 3: Monter la structure du répertoire service/ et composer.json
...
Attendez — réfléchissons à nouveau. service/ doit avoir son PROPRE répertoire vendor pour ses propres dépendances, mais il peut partager le vendor d'admin ou être installé séparément. L'approche la plus simple : service/ fait `composer install` indépendamment, et référence common/ via la configuration d'autoload partagée.

Je devrais simplifier : service/ fait `composer install` indépendamment. Le répertoire common/ est référencé via le mapping d'autoload PSR-4. admin et service pointent tous deux vers `../common/model` et `../common/middleware`.

En fait, avec webman v2, le plus simple est :
- service/ possède son propre vendor/ (via composer install)
- Les modèles communs sont autoloadés via PSR-4 dans composer.json : `"common\\model\\": "../common/model"`
- Pas besoin de symlink

Écrivons cela correctement.
...
### Task 4: Créer les fichiers de configuration de service/
...
Remarque : service/ a besoin de ses propres copies des middlewares Cors, SecurityFilter, RateLimit OU de référencer ceux d'admin. Pour l'instant, faites des copies dans service/app/middleware/.
...
### Task 5: Copier les middlewares partagés dans service/ et créer UserAuth
...
### Task 6: Créer la migration SQL des 12 tables de la version de base
...
## Phase 3 : Couche partagée — Modèles

### Task 7: Créer common/model/User.php
...
### Task 8: Créer common/model/UserWallet.php (avec verrou optimiste)
...
### Task 9: Créer les 10 modèles restants
...
## Phase 4 : Contrôleurs API côté C

### Task 10: Créer le BaseController du service et copier les services partagés
...
### Task 11: Créer AuthController (register/login/refresh)
...
### Task 12: Créer WalletController
...
### Task 13: Créer ExchangeController (pièces de plateforme ⇄ pièces de jeu)
...
### Task 14: Créer DepositController et WithdrawController
...
### Task 15: Créer GameController et UserController côté C
...
## Phase 5 : Extension du back-office

### Task 16: Créer le GameController d'admin (CRUD)
...
### Task 17: Créer le WithdrawController d'admin (review + switch + limits)
...
### Task 18: Créer PlatformUserController, PaymentController, AnnouncementController + étendre Dashboard et Export
...
## Phase 6 : Ajout de l'API d'annonces côté C

### Task 19: Créer l'AnnouncementController côté C
...
## Phase 7 : Intégration finale des routes et validation

### Task 20: Intégration finale de service/config/route.php
...
## Phase 8 : Frontend Flutter (résumé)

### Task 21-25: Extension du back-office Flutter PC + création de la plateforme côté C

**Note:** Full Flutter implementation details to be expanded in a follow-up plan. This section summarizes the pages needed.

**admin Flutter (extension) :**
- `GameListPage` — liste/création/édition de jeux, gestion des devises
- `WithdrawPage` — liste des ordres de retrait / bouton de vérification / interrupteur / réglage des limites
- `PlatformUserPage` — liste/détail/bannissement des utilisateurs côté C
- `PaymentPage` — gestion des modes de paiement
- `AnnouncementPage` — gestion des annonces
- `DashboardPage` — extension du panneau du tableau de bord de la plateforme
- Boutons d'export dans les pages de liste

**platform Flutter (nouvelle, style PC) :**
- `LoginPage` — connexion/inscription
- `GameHallPage` — hall des jeux (liste/recherche)
- `GameDetailPage` — détail du jeu/lancement
- `WalletPage` — solde du portefeuille/transactions
- `DepositPage` — dépôt
- `ExchangePage` — conversion
- `WithdrawPage` — retrait
- `ProfilePage` — espace personnel

---

## Self-Review

1. **Couverture des specs :** chaque exigence de la spec a des tâches correspondantes :
   - Infrastructure → Tâches 1-5
   - Tables de base de données → Tâche 6
   - Modèles → Tâches 7-9
   - Auth côté C → Tâche 11
   - Portefeuille côté C → Tâche 12
   - Conversion côté C → Tâche 13
   - Dépôt/Retrait côté C → Tâche 14
   - Jeu/Utilisateur côté C → Tâche 15
   - CRUD jeux admin → Tâche 16
   - Vérification/interrupteur des retraits admin → Tâche 17
   - Utilisateurs de plateforme/paiements/annonces admin → Tâche 18
   - Frontend Flutter → Tâches 21-25 (résumé)

2. **Recherche de placeholders :** aucun TBD, TODO ou code incomplet. Toutes les tâches ont des blocs de code complets.

3. **Cohérence des types :**
   - Tous les IDs sont BIGINT → Snowflake::generate()
   - Les IDs d'API sont des chaînes hashid → encodeId/decodeId
   - Les montants monétaires utilisent la représentation en chaîne pour la précision décimale → bcmath
   - Les opérations de portefeuille utilisent le verrou optimiste → champ version
