# Platform Agregasi Game Global Versi Dasar — Rencana Implementasi
<!-- lang-nav -->

Languages: [中文](2026-05-22-game-platform-plan.md) · [English](2026-05-22-game-platform-plan.en.md) · [한국어](2026-05-22-game-platform-plan.ko.md) · [Русский](2026-05-22-game-platform-plan.ru.md) · [Deutsch](2026-05-22-game-platform-plan.de.md) · [Français](2026-05-22-game-platform-plan.fr.md) · [Español](2026-05-22-game-platform-plan.es.md) · [Português](2026-05-22-game-platform-plan.pt.md) · [हिन्दी](2026-05-22-game-platform-plan.hi.md) · [العربية](2026-05-22-game-platform-plan.ar.md) · [বাংলা](2026-05-22-game-platform-plan.bn.md) · **Bahasa Indonesia** · [日本語](2026-05-22-game-platform-plan.ja.md)


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Tujuan:** Menjalankan loop inti: registrasi → deposit → penukaran → koin game → penarikan → review admin

**Arsitektur:** admin/ (backend manajemen) dan service/ (bisnis sisi C) adalah instance webman v2 independen, berbagi Model dan Service melalui common/ (PSR-4 autoload). Migrasi database terpusat di admin/database/migrations/. Frontend Flutter PC backend manajemen diperluas + platform PC sisi C baru dibuat.

**Tech Stack:** PHP 8.3+, webman v2, MySQL 8.0, Redis, rangkaian komponen erikwang2013/*
...
- `common/model/` — Model Eloquent, berkorespondensi satu-ke-satu dengan tabel data, mendefinisikan casts/relations
- `service/app/api/v1/controller/` — API sisi C, menangani permintaan pengguna, memanggil Model
- `admin/app/admin/controller/` — API backend manajemen, operasi administrator
- `common/middleware/UserAuth.php` — middleware verifikasi JWT sisi C, menyuntikkan userId

---

## Phase 1: Pembangunan Infrastruktur

### Task 1: Buat common/composer.json dan definisikan namespace
...
### Task 2: Daftarkan common/ ke admin dan buat konfigurasi autoload service/
...
### Task 3: Bangun struktur direktori service/ dan composer.json
...
### Task 4: Buat file konfigurasi service/
...
### Task 5: Salin middleware bersama ke service/ dan buat UserAuth
...
### Task 6: Buat migrasi SQL untuk 12 tabel versi dasar
...
## Phase 3: Lapisan Bersama — Models

### Task 7: Buat common/model/User.php
...
### Task 8: Buat common/model/UserWallet.php (dengan optimistic lock)
...
## Phase 4: Controller API Sisi C

### Task 10: Buat BaseController service dan salin layanan bersama
...
### Task 11: Buat AuthController (register/login/refresh)
...
### Task 13: Buat ExchangeController (koin platform ⇄ koin game)
...
### Task 14: Buat DepositController dan WithdrawController
...
### Task 15: Buat GameController, UserController untuk sisi C
...
## Phase 5: Perluasan Backend Manajemen

### Task 16: Buat admin GameController (CRUD)
...
### Task 17: Buat admin WithdrawController (review + switch + limits)
...
### Task 18: Buat PlatformUserController, PaymentController, AnnouncementController + perluas Dashboard dan Export
...
## Phase 6: Tambah Interface Pengumuman Sisi C

### Task 19: Buat AnnouncementController sisi C
...
## Phase 7: Integrasi dan Verifikasi Rute Akhir

### Task 20: Integrasi akhir service/config/route.php
...
## Phase 8: Frontend Flutter (Ringkasan)

### Task 21-25: Perluasan backend manajemen Flutter PC + pembuatan platform sisi C

**Note:** Full Flutter implementation details to be expanded in a follow-up plan. This section summarizes the pages needed.

**admin Flutter (perluasan):**
- `GameListPage` — daftar/buat/edit game, manajemen mata uang
- `WithdrawPage` — daftar order penarikan/tombol review/saklar/pengaturan limit
- `PlatformUserPage` — daftar/detail/ban pengguna sisi C
- `PaymentPage` — manajemen metode pembayaran
- `AnnouncementPage` — manajemen pengumuman
- `DashboardPage` — panel dasbor platform yang diperluas
- Export buttons in list pages

**platform Flutter (baru, gaya PC):**
- `LoginPage` — login/registrasi
- `GameHallPage` — lobi game (daftar/pencarian)
- `GameDetailPage` — detail game/launch
- `WalletPage` — saldo dompet/riwayat
- `DepositPage` — deposit
- `ExchangePage` — penukaran
- `WithdrawPage` — penarikan
- `ProfilePage` — pusat pribadi

---

## Self-Review

1. **Cakupan spec:** Setiap kebutuhan spec memiliki task terkait:
   - Infrastruktur → Tasks 1-5
   - Tabel database → Task 6
   - Models → Tasks 7-9
   - Auth sisi C → Task 11
   - Wallet sisi C → Task 12
   - Exchange sisi C → Task 13
   - Deposit/Withdraw sisi C → Task 14
   - Game/User sisi C → Task 15
   - Admin Game CRUD → Task 16
   - Review/saklar withdraw admin → Task 17
   - Pengguna/pembayaran/pengumuman platform admin → Task 18
   - Frontend Flutter → Tasks 21-25 (dirangkum)

2. **Pemindaian placeholder:** Tidak ada TBD, TODO, atau kode tidak lengkap. Semua task memiliki blok kode lengkap.

3. **Konsistensi tipe:**
   - Semua ID adalah BIGINT → Snowflake::generate()
   - ID API adalah string hashid → encodeId/decodeId
   - Jumlah uang menggunakan representasi string untuk presisi desimal → bcmath
   - Operasi dompet menggunakan optimistic locking → field version
   - Autentikasi JWT → jwt()->create/verify
   - Namespace model → common\model\
   - Namespace middleware → common\middleware\
   - Namespace controller → app\api\v1\controller\ (service) dan app\admin\controller\ (admin)
