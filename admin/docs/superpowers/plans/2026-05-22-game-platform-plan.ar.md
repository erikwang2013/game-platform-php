# منصة تجميع الألعاب العالمية (الإصدار الأساسي) — خطة التنفيذ
<!-- lang-nav -->
...
**Goal:** تشغيل الحلقة الأساسية: تسجيل → شحن → استبدال → عملات ألعاب → سحب → مراجعة الخلفية

**Architecture:** admin/ (لوحة الإدارة) و service/ (أعمال الطرف C) مثيلان مستقلان من webman v2، يشتركان في Model و Service عبر common/ (PSR-4 autoload). تُوحَّد ترحيلات قاعدة البيانات في admin/database/migrations/. الواجهة الأمامية: توسيع Flutter PC للوحة الإدارة + إنشاء منصة الطرف C على PC.

**Tech Stack:** PHP 8.3+, webman v2, MySQL 8.0, Redis, مكونات سلسلة erikwang2013/*
...
│   ├── composer.json
│   ├── model/
│   │   ├── User.php                    # مستخدمو الطرف C
│   │   ├── UserWallet.php              # محفظة عملات المنصة (مع قفل تفاؤلي)
│   │   ├── UserGameWallet.php          # محفظة عملات الألعاب
│   │   ├── Game.php                    # الألعاب
│   │   ├── GameCurrency.php            # عملات الألعاب
│   │   ├── DepositOrder.php            # طلبات الشحن
│   │   ├── WithdrawOrder.php           # طلبات السحب
│   │   ├── ExchangeRecord.php          # سجلات الاستبدال
│   │   ├── Transaction.php             # حركات المنصة
│   │   ├── PaymentMethod.php           # طرق الدفع
│   │   ├── Announcement.php            # الإعلانات
│   │   └── PlatformConfig.php          # إعدادات المنصة
│   └── middleware/
│       └── UserAuth.php                # وسيطة مصادقة مستخدمي الطرف C عبر JWT
│
├── service/
│   ├── composer.json
│   ├── start.php
...
│
├── admin/
│   ├── composer.json                  # إضافة common/ path repo + وحدات تحكم جديدة
│   ├── app/admin/controller/
│   │   ├── GameController.php         # جديد
│   │   ├── WithdrawController.php     # جديد
│   │   ├── PaymentController.php      # جديد
│   │   ├── PlatformUserController.php # جديد
│   │   ├── AnnouncementController.php # جديد
│   │   ├── DashboardController.php    # توسيع لوحة تحكم المنصة
│   │   └── ExportController.php       # توسيع تصدير الحركات/السحب
│   ├── app/model/ -> ../../common/model/  (symlink, or autoload)
│   ├── config/route.php               # توسيع المسارات الجديدة
│   ├── config/autoload.php            # توسيع common namespace
│   └── database/migrations/
│       └── 2026_05_22_000003_platform_tables.sql  # جداول الإصدار الأساسي الـ12
│
└── apps/flutter/
    ├── admin/                          # لوحة إدارة PC (توسيع القائمة)
    └── platform/                       # منصة مستخدمي الطرف C على PC (جديدة)
```

**Responsibility boundaries:**
- `common/model/` — نماذج Eloquent، بمراسلة واحد-لواحد مع جداول البيانات، تعرّف casts/relations
- `service/app/api/v1/controller/` — API الطرف C، معالجة طلبات المستخدمين، استدعاء النماذج
- `admin/app/admin/controller/` — API لوحة الإدارة، عمليات المشرفين
- `common/middleware/UserAuth.php` — وسيطة تحقق JWT للطرف C، حقن userId

---

## Phase 1: البنية التحتية

### Task 1: إنشاء common/composer.json وتعريف النطاقات
...
### Task 2: تسجيل common/ في admin وإنشاء إعداد autoload لـ service/
...
### Task 3: بناء هيكل service/ و composer.json
...
### Task 4: إنشاء ملفات إعدادات service/
...
### Task 5: نسخ الوسائط المشتركة إلى service/ وإنشاء UserAuth
...
## Phase 2: ترحيل قاعدة البيانات

### Task 6: إنشاء ترحيل SQL لجداول الإصدار الأساسي الـ12
...
## Phase 3: الطبقة المشتركة — النماذج
...
## Phase 4: وحدات تحكم API للطرف C
...
## Phase 6: إضافة واجهة الإعلانات للطرف C
...
## Phase 7: الدمج النهائي للمسارات والتحقق

### Task 20: الدمج النهائي لـ service/config/route.php
...
## Phase 8: الواجهة الأمامية Flutter (ملخص)

### Task 21-25: توسيع لوحة إدارة Flutter PC + إنشاء منصة الطرف C

**Note:** Full Flutter implementation details to be expanded in a follow-up plan. This section summarizes the pages needed.

**admin Flutter (توسيع):**
- `GameListPage` — قائمة الألعاب/إنشاء/تعديل/إدارة العملات
- `WithdrawPage` — قائمة طلبات السحب/زر المراجعة/المفتاح/إعدادات الحدود
- `PlatformUserPage` — قائمة مستخدمي الطرف C/التفاصيل/الحظر
- `PaymentPage` — إدارة طرق الدفع
- `AnnouncementPage` — إدارة الإعلانات
- `DashboardPage` — توسيع لوحة تحكم المنصة
- Export buttons in list pages

**platform Flutter (جديد بأسلوب PC):**
- `LoginPage` — تسجيل الدخول/التسجيل
- `GameHallPage` — قاعة الألعاب (قائمة/بحث)
- `GameDetailPage` — تفاصيل اللعبة/البدء
- `WalletPage` — رصيد المحفظة/الحركات
- `DepositPage` — الشحن
- `ExchangePage` — الاستبدال
- `WithdrawPage` — السحب
