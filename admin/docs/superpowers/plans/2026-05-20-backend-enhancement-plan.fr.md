# Amélioration du backend — Plan d'implémentation
<!-- lang-nav -->

Languages: [中文](2026-05-20-backend-enhancement-plan.md) · [English](2026-05-20-backend-enhancement-plan.en.md) · [한국어](2026-05-20-backend-enhancement-plan.ko.md) · [Русский](2026-05-20-backend-enhancement-plan.ru.md) · [Deutsch](2026-05-20-backend-enhancement-plan.de.md) · **Français** · [Español](2026-05-20-backend-enhancement-plan.es.md) · [Português](2026-05-20-backend-enhancement-plan.pt.md) · [हिन्दी](2026-05-20-backend-enhancement-plan.hi.md) · [العربية](2026-05-20-backend-enhancement-plan.ar.md) · [বাংলা](2026-05-20-backend-enhancement-plan.bn.md) · [Bahasa Indonesia](2026-05-20-backend-enhancement-plan.id.md) · [日本語](2026-05-20-backend-enhancement-plan.ja.md)


> **Pour les travailleurs agentiques :** SOUS-COMPÉTENCE REQUISE : utilisez superpowers:subagent-driven-development (recommandé) ou superpowers:executing-plans pour implémenter ce plan tâche par tâche. Les étapes utilisent la syntaxe des cases à cocher (`- [ ]`) pour le suivi.

**Objectif :** implémenter 13 améliorations du backend : 3 nouveaux middlewares (CORS, RateLimit, OperationLog), 6 nouveaux contrôleurs, 2 modifications de modèles, routage + configuration des middlewares.

**Architecture :** suivre les modèles webman existants — les middlewares implémentent `MiddlewareInterface::process()`, les contrôleurs héritent de `BaseController`, les routes sont définies par closures ou par `[class, method]`. Les middlewares globaux sont enregistrés dans `config/middleware.php`, les middlewares au niveau des routes sont montés sur les groupes de routes dans `config/route.php`.

**Pile technique :** PHP 8.3+, webman v2, Redis (extension), PhpSpreadsheet (import)

**Ordre des dépendances :** modifications des modèles → middlewares → contrôleurs → routes/config

---

### Tâche 1 : Ajout des traits SoftDeletes + Searchable au modèle AdminUser

**Fichiers :**
- Modifier : `app/model/AdminUser.php`

- [ ] **Étape 1 : modifier le modèle AdminUser**

Insérer deux imports `use` après la ligne 10 de `app/model/AdminUser.php` et utiliser les deux traits en tête de corps de classe :
...
Ajouter également, à la fin de la classe (après la méthode `roles()`, avant l'accolade de fermeture), la méthode `toSearchableArray()` :
...
### Tâche 2 : Middleware CORS

**Fichiers :**
- Créer : `app/middleware/Cors.php`

- [ ] **Étape 1 : créer le middleware Cors**
...
### Tâche 3 : Middleware de limitation RateLimit

**Fichiers :**
- Créer : `app/middleware/RateLimit.php`

- [ ] **Étape 1 : créer le middleware RateLimit**
...
### Tâche 4 : Middleware de journal d'opérations OperationLog

**Fichiers :**
- Créer : `app/middleware/OperationLog.php`

- [ ] **Étape 1 : créer le middleware OperationLog**
...
### Tâche 5 : Configuration des middlewares globaux

**Fichiers :**
- Modifier : `config/middleware.php`

- [ ] **Étape 1 : enregistrer les middlewares globaux**

Remplacer le contenu de `config/middleware.php` par :
...
### Tâche 6 : Contrôleur de health check

**Fichiers :**
- Créer : `app/admin/controller/HealthController.php`

- [ ] **Étape 1 : créer HealthController**
...
### Tâche 7 : Contrôleur CRUD de configuration système

**Fichiers :**
- Créer : `app/admin/controller/ConfigController.php`

- [ ] **Étape 1 : créer ConfigController**
...
### Tâche 8 : Contrôleur de consultation des journaux d'opérations

**Fichiers :**
- Créer : `app/admin/controller/LogController.php`

- [ ] **Étape 1 : créer LogController**
...
### Tâche 9 : Contrôleur d'espace personnel (avec déconnexion)

**Fichiers :**
- Créer : `app/admin/controller/ProfileController.php`
- Modifier : `app/middleware/AdminAuth.php` (vérification de la liste noire JWT)

- [ ] **Étape 1 : modifier le middleware AdminAuth pour ajouter la vérification de la liste noire JWT**

Dans la méthode `AdminAuth::process()`, insérer la vérification de la liste noire après l'extraction du token et avant le décodage JWT :
...
Ajouter également l'import Redis. Modifier l'en-tête du fichier :
...
- [ ] **Étape 2 : créer ProfileController**
...
### Tâche 10 : Contrôleur d'upload de fichiers

**Fichiers :**
- Créer : `app/admin/controller/UploadController.php`

- [ ] **Étape 1 : créer UploadController**
...
### Tâche 11 : Opérations par lots sur les utilisateurs

**Fichiers :**
- Modifier : `app/admin/controller/UserController.php`

- [ ] **Étape 1 : ajouter deux méthodes à la fin de la classe UserController (après la méthode `destroy`)**
...
### Tâche 12 : Contrôleur d'import Excel

**Fichiers :**
- Créer : `app/admin/controller/ImportController.php`

- [ ] **Étape 1 : créer ImportController**
...
### Tâche 13 : Mise à jour du routage — toutes les nouvelles routes + liaison des middlewares

**Fichiers :**
- Modifier : `config/route.php`

- [ ] **Étape 1 : mettre à jour la configuration des routes**

Ajouter de nouvelles routes dans le groupe de routes `/admin` et ajouter le middleware OperationLog. Fichier de routes complet :
...
Après l'implémentation, vérifier point par point :
