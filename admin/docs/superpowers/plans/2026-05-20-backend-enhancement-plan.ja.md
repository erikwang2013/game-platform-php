...
**Goal:** バックエンド強化 13 項目を実装する: 新しいミドルウェア 3 つ (CORS, RateLimit, OperationLog)、新しいコントローラー 6 つ、モデル修正 2 件、ルート + ミドルウェア設定。

**Architecture:** 既存の webman パターンに従う — ミドルウェアは `MiddlewareInterface::process()` を実装し、コントローラーは `BaseController` を継承し、ルートはクロージャまたは `[class, method]` で定義する。グローバルミドルウェアは `config/middleware.php` に登録し、ルートレベルのミドルウェアは `config/route.php` のルートグループにマウントする。

**Tech Stack:** PHP 8.3+, webman v2, Redis (拡張), PhpSpreadsheet (インポート)

**依存順序:** モデル修正 → ミドルウェア → コントローラー → ルート/設定
...
`app/model/AdminUser.php` の 10 行目以降に 2 つの `use` インポートを挿入し、クラス本体内の先頭で 2 つの trait を使用する:
...
また、クラスの末尾（`roles()` メソッドの後、クラスの閉じ括弧の前）に `toSearchableArray()` メソッドを追加する:
...
`config/middleware.php` の内容を次のように変更する:
...
### Task 9: プロフィールコントローラー（ログアウト含む）

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT ブラックリスト検証)

- [ ] **Step 1: AdminAuth ミドルウェアを修正し、JWT ブラックリスト検証を追加**

`AdminAuth::process()` メソッド内で、token 抽出後、JWT デコード前にブラックリストチェックを挿入する:
...
同時に Redis インポートを追加する。ファイルヘッダを修正:
...
`/admin` ルートグループ内に新しいルートを追加し、同時に OperationLog ミドルウェアを追記する。完全なルートファイル:
