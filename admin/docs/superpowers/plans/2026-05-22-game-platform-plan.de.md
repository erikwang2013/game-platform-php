# Globale Spielaggregationsplattform Basisversion — Implementierungsplan
<!-- lang-nav -->
...
- `common/model/` — Eloquent-Modelle, eins zu eins zugeordnet zu den Datenbanktabellen, definieren casts/relations
- `service/app/api/v1/controller/` — C-End-API, verarbeitet Benutzeranfragen, ruft Modelle auf
- `admin/app/admin/controller/` — Verwaltungs-Backend-API, Administratoroperationen
- `common/middleware/UserAuth.php` — C-End-JWT-Verifizierungsmiddleware, injiziert userId

---

## Phase 1: Infrastruktur-Aufbau

### Aufgabe 1: common/composer.json erstellen und Namespace definieren

**Dateien:**
- Erstellen: `common/composer.json`

- [ ] **Schritt 1: common/composer.json schreiben**
...
- [ ] **Schritt 2: composer validate in common/ ausführen**

Ausführen: `cd common && composer validate`
Erwartet: "valid" (Warnung zu fehlender Version kann ignoriert werden)
...
### Aufgabe 2: common/ in admin registrieren und service/-Autoload-Konfiguration erstellen

**Dateien:**
- Ändern: `admin/composer.json`
- Ändern: `admin/config/autoload.php`

- [ ] **Schritt 1: common path repository zu admin/composer.json hinzufügen**

In `admin/composer.json` im Block `"autoload"` → `"psr-4"` ergänzen:

Der bestehende Block ist:
```json
"autoload": {
  "psr-4": {
    "": "./",
...
- [ ] **Schritt 2: composer dump-autoload ausführen**

Ausführen: `cd admin && composer dump-autoload`
Erwartet: erfolgreich, keine Fehler

- [ ] **Schritt 3: common-Autoload zu admin/config/autoload.php hinzufügen (falls nötig)**

Zuerst die aktuelle Datei lesen. Falls common nicht bereits geladen wird, die Namespace-Zuordnung ergänzen.
...
### Aufgabe 3: service/-Verzeichnisstruktur und composer.json aufbauen

**Dateien:**
- Erstellen: `service/composer.json`
- Erstellen: `service/.env.example`
- Erstellen: `service/start.php`
- Erstellen: `service/windows.php`
- Erstellen: `service/support/bootstrap.php`
- Erstellen: `service/support/Request.php`
- Erstellen: `service/support/Response.php`
- Erstellen: `service/support/Setup.php`
- Erstellen: `service/app/functions.php`

- [ ] **Schritt 1: service/composer.json schreiben**
...
- [ ] **Schritt 2: service/start.php schreiben**
...
Moment — nochmal überdenken. service/ sollte ein EIGENES vendor-Verzeichnis für die eigenen Abhängigkeiten haben, kann aber das vendor von admin mitnutzen oder separat installieren. Der einfachste Ansatz: service/ führt `composer install` unabhängig durch. Das common/-Verzeichnis wird über die PSR-4-Autoload-Zuordnung referenziert. Sowohl admin als auch service zeigen auf `../common/model` und `../common/middleware`.

Eigentlich ist mit webman v2 das Einfachste:
- service/ hat ein eigenes vendor/ (aus composer install)
- Gemeinsame Modelle werden per PSR-4 in composer.json geladen: `"common\\model\\": "../common/model"`
- Kein Symlink nötig

Schreiben wir das korrekt.
...
- [ ] **Schritt 3: service/windows.php schreiben** (gleicher Inhalt wie das Muster von admin/windows.php)
...
- [ ] **Schritt 4: service/.env.example schreiben**
...
- [ ] **Schritt 5: service/support/bootstrap.php schreiben**
...
(Minimal — webman erkennt support/bootstrap.php automatisch)

- [ ] **Schritt 6: service/app/functions.php schreiben**
...
- [ ] **Schritt 7: support-Dateien aus admin kopieren**

Ausführen: `cp admin/support/Request.php service/support/Request.php`
Ausführen: `cp admin/support/Response.php service/support/Response.php`
Ausführen: `cp admin/support/Setup.php service/support/Setup.php`

- [ ] **Schritt 8: composer-Abhängigkeiten für service installieren**
...
### Aufgabe 4: service/-Konfigurationsdateien erstellen

**Dateien:**
- Erstellen: `service/config/app.php`
- Erstellen: `service/config/autoload.php`
- Erstellen: `service/config/bootstrap.php`
- Erstellen: `service/config/container.php`
- Erstellen: `service/config/database.php`
- Erstellen: `service/config/dependence.php`
- Erstellen: `service/config/exception.php`
- Erstellen: `service/config/hashids.php`
- Erstellen: `service/config/jwt.php`
- Erstellen: `service/config/log.php`
- Erstellen: `service/config/middleware.php`
- Erstellen: `service/config/process.php`
- Erstellen: `service/config/route.php`
- Erstellen: `service/config/server.php`
- Erstellen: `service/config/session.php`
- Erstellen: `service/config/snowflake.php`
- Erstellen: `service/config/static.php`
- Erstellen: `service/config/translation.php`
- Erstellen: `service/config/view.php`
- Erstellen: `service/config/plugin/erikwang2013/hashids/app.php`
- Erstellen: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Erstellen: `service/config/plugin/erikwang2013/jwt/app.php`

- [ ] **Schritt 1: Basiskonfigurationen aus admin kopieren und anpassen**

Alle Konfigurationsdateien aus `admin/config/` nach `service/config/` kopieren und die Werte anpassen:

Wichtige Unterschiede in service/config/:

`database.php` — gleiche DB, gleiche Verbindung
`route.php` — andere Routen (C端 /api/*)
`middleware.php` — andere Middleware-Kette
`server.php` — anderer Port (8788 statt 8787)
`snowflake.php` — andere datacenter_id/worker_id (datacenter_id=1, worker_id=2)

- [ ] **Schritt 2: service/config/middleware.php schreiben**
...
Hinweis: service benötigt eigene Kopien der Middlewares Cors, SecurityFilter, RateLimit ODER referenziert die von admin. Vorerst Kopien in service/app/middleware/ anlegen.

- [ ] **Schritt 3: service/config/route.php schreiben** (Grundgerüst)
...
- [ ] **Schritt 4: Plugin-Konfigurationen kopieren**

Ausführen: `cp -r admin/config/plugin service/config/plugin`
...
### Aufgabe 5: Gemeinsame Middlewares nach service/ kopieren und UserAuth erstellen

**Dateien:**
- Erstellen: `service/app/middleware/Cors.php`
- Erstellen: `service/app/middleware/SecurityFilter.php`
- Erstellen: `service/app/middleware/RateLimit.php`
- Erstellen: `service/app/middleware/ApiVersion.php`
- Erstellen: `common/middleware/UserAuth.php`

- [ ] **Schritt 1: Middleware-Dateien aus admin kopieren**

Ausführen: `cp admin/app/middleware/Cors.php service/app/middleware/Cors.php`
Ausführen: `cp admin/app/middleware/SecurityFilter.php service/app/middleware/SecurityFilter.php`
Ausführen: `cp admin/app/middleware/RateLimit.php service/app/middleware/RateLimit.php`
Ausführen: `cp admin/app/middleware/ApiVersion.php service/app/middleware/ApiVersion.php`

- [ ] **Schritt 2: Namespace in kopierten Middlewares aktualisieren**

`namespace app\middleware;` in `namespace app\middleware;` ändern (gleich, da service denselben Namespace verwendet).

Eigentlich den Namespace prüfen — da service/ `"app\\": "./app"` per Autoload lädt, bleibt der Namespace `app\middleware`.

- [ ] **Schritt 3: common/middleware/UserAuth.php erstellen**
...
- [ ] **Schritt 3: Vorhandensein der Tabellen prüfen**

```bash
mysql -h 127.0.0.1 -u root game_platform -e "SHOW TABLES LIKE 'erik_%';"
```

Erwartet: Liste enthält alle 12 neuen Tabellen + die bestehenden admin-Tabellen.
...
## Phase 3: Gemeinsame Ebene — Modelle

### Aufgabe 7: common/model/User.php erstellen

**Dateien:**
- Erstellen: `common/model/User.php`

- [ ] **Schritt 1: Sicherstellen, dass das vendor-Verzeichnis von common eingerichtet ist**

Ausführen: `cd common && composer install 2>/dev/null; echo "OK"`

- [ ] **Schritt 2: common/model/User.php schreiben**
...
### Aufgabe 8: common/model/UserWallet.php erstellen (mit Optimistic Lock)

**Dateien:**
- Erstellen: `common/model/UserWallet.php`

- [ ] **Schritt 1: UserWallet.php schreiben**
...
### Aufgabe 9: Die übrigen 10 Modelle erstellen

**Dateien:**
- Erstellen: `common/model/UserGameWallet.php`
- Erstellen: `common/model/Game.php`
- Erstellen: `common/model/GameCurrency.php`
- Erstellen: `common/model/DepositOrder.php`
- Erstellen: `common/model/WithdrawOrder.php`
- Erstellen: `common/model/ExchangeRecord.php`
- Erstellen: `common/model/Transaction.php`
- Erstellen: `common/model/PaymentMethod.php`
- Erstellen: `common/model/Announcement.php`
- Erstellen: `common/model/PlatformConfig.php`

- [ ] **Schritt 1: UserGameWallet.php schreiben**
...
- [ ] **Schritt 3: GameCurrency.php schreiben**
...
- [ ] **Schritt 4: DepositOrder.php schreiben**
...
- [ ] **Schritt 5: WithdrawOrder.php schreiben**
...
- [ ] **Schritt 6: ExchangeRecord.php schreiben**
...
- [ ] **Schritt 7: Transaction.php schreiben**
...
- [ ] **Schritt 8: PaymentMethod.php schreiben**
...
- [ ] **Schritt 9: Announcement.php schreiben**
...
- [ ] **Schritt 10: PlatformConfig.php schreiben**
...
## Phase 4: C-End-API-Controller

### Aufgabe 10: service-BaseController erstellen und gemeinsame Services kopieren

**Dateien:**
- Erstellen: `service/app/api/v1/controller/BaseController.php`
- Erstellen: `service/app/common/HashidsService.php`
- Erstellen: `service/app/common/SnowflakeService.php`
- Erstellen: `service/app/common/EncryptionService.php`
- Ändern: `service/config/route.php`

- [ ] **Schritt 1: Service-Klassen kopieren und Namespace anpassen**

Ausführen: `cp admin/app/common/HashidsService.php service/app/common/HashidsService.php`
Ausführen: `cp admin/app/common/SnowflakeService.php service/app/common/SnowflakeService.php`
Ausführen: `cp admin/app/common/EncryptionService.php service/app/common/EncryptionService.php`

Namespace aktualisieren: `app\common\HashidsService` → ist bereits `app\common`, keine Änderung nötig, da service/ `app\\` → `./app` per Autoload lädt.

- [ ] **Schritt 2: BaseController.php schreiben**
...
### Aufgabe 11: AuthController erstellen (register/login/refresh)

**Dateien:**
- Erstellen: `service/app/api/v1/controller/AuthController.php`
- Ändern: `service/config/route.php`

- [ ] **Schritt 1: AuthController.php schreiben**
...
- [ ] **Schritt 2: service/config/route.php aktualisieren** — Auth-Routen hinzufügen:
...
### Aufgabe 12: WalletController erstellen

**Dateien:**
- Erstellen: `service/app/api/v1/controller/WalletController.php`
- Ändern: `service/config/route.php`

- [ ] **Schritt 1: WalletController.php schreiben**
...
- [ ] **Schritt 2: service/config/route.php aktualisieren** — unter der authentifizierten Gruppe hinzufügen:
...
### Aufgabe 13: ExchangeController erstellen (Plattformwährung ⇄ Spielwährung)

**Dateien:**
- Erstellen: `service/app/api/v1/controller/ExchangeController.php`
- Ändern: `service/config/route.php`

- [ ] **Schritt 1: ExchangeController.php schreiben**
...
- [ ] **Schritt 2: route.php aktualisieren — Exchange-Routen unter der authentifizierten Gruppe hinzufügen**
...
### Aufgabe 14: DepositController und WithdrawController erstellen

**Dateien:**
- Erstellen: `service/app/api/v1/controller/DepositController.php`
- Erstellen: `service/app/api/v1/controller/WithdrawController.php`
- Ändern: `service/config/route.php`

- [ ] **Schritt 1: DepositController.php schreiben**
...
- [ ] **Schritt 2: WithdrawController.php schreiben**
...
- [ ] **Schritt 3: route.php aktualisieren — Einzahlungs- und Auszahlungsrouten hinzufügen**
...
### Aufgabe 15: GameController und UserController für C端 erstellen

**Dateien:**
- Erstellen: `service/app/api/v1/controller/GameController.php`
- Erstellen: `service/app/api/v1/controller/UserController.php`
- Ändern: `service/config/route.php`

- [ ] **Schritt 1: GameController.php schreiben**
...
- [ ] **Schritt 2: UserController.php schreiben**
...
- [ ] **Schritt 3: route.php aktualisieren** — Spiel- und Benutzerrouten hinzufügen
...
## Phase 5: Erweiterung des Verwaltungs-Backends

### Aufgabe 16: admin-GameController erstellen (CRUD)

**Dateien:**
- Erstellen: `admin/app/admin/controller/GameController.php`
- Ändern: `admin/config/route.php`

- [ ] **Schritt 1: admin GameController.php schreiben**
...
- [ ] **Schritt 2: admin/config/route.php aktualisieren** — Spielrouten in der /admin-Gruppe hinzufügen:
...
### Aufgabe 17: admin-WithdrawController erstellen (Prüfung + Schalter + Limits)

**Dateien:**
- Erstellen: `admin/app/admin/controller/WithdrawController.php`
- Ändern: `admin/config/route.php`

- [ ] **Schritt 1: admin WithdrawController.php schreiben**
...
- [ ] **Schritt 2: route.php aktualisieren** — Auszahlungsrouten hinzufügen
...
### Aufgabe 18: PlatformUserController, PaymentController, AnnouncementController erstellen + Dashboard und Export erweitern

**Dateien:**
- Erstellen: `admin/app/admin/controller/PlatformUserController.php`
- Erstellen: `admin/app/admin/controller/PaymentController.php`
- Erstellen: `admin/app/admin/controller/AnnouncementController.php`
- Ändern: `admin/app/admin/controller/DashboardController.php`
- Ändern: `admin/app/admin/controller/ExportController.php`
- Ändern: `admin/config/route.php`

- [ ] **Schritt 1: PlatformUserController.php schreiben**
...
- [ ] **Schritt 2: PaymentController.php schreiben** (minimal für MVP — Methoden auflisten + Umschalten)
...
- [ ] **Schritt 3: AnnouncementController.php schreiben**
...
- [ ] **Schritt 4: DashboardController erweitern — platformDashboard-Methode hinzufügen**
...
- [ ] **Schritt 5: ExportController erweitern — Benutzer- und Transaktionsexporte hinzufügen**
...
- [ ] **Schritt 6: route.php aktualisieren** — alle neuen admin-Routen hinzufügen
...
## Phase 6: C-End-Schnittstelle für Ankündigungen hinzufügen

### Aufgabe 19: C端-AnnouncementController erstellen

**Dateien:**
- Erstellen: `service/app/api/v1/controller/AnnouncementController.php`
- Ändern: `service/config/route.php`

- [ ] **Schritt 1: AnnouncementController.php schreiben**
...
- [ ] **Schritt 2: route.php aktualisieren**
...
## Phase 7: Finale Routen-Integration und Verifikation

### Aufgabe 20: Finale service/config/route.php-Integration

- [ ] **Schritt 1: Die vollständige service/config/route.php schreiben**
...
- [ ] **Schritt 2: Prüfen, dass service/ starten kann**
...
- [ ] **Schritt 3: Einen öffentlichen Endpunkt testen**

```bash
curl -s http://localhost:8788/health | python3 -m json.tool
```

Erwartet: `{"code": 0, "message": "ok"}`
...
## Phase 8: Flutter-Frontend (Zusammenfassung)

### Aufgabe 21-25: Flutter-PC-Verwaltungs-Backend erweitern + C端-Plattform neu erstellen

**Hinweis:** Die vollständigen Flutter-Implementierungsdetails werden in einem Folgeplan ausgearbeitet. Dieser Abschnitt fasst die benötigten Seiten zusammen.

**admin Flutter (Erweiterung):**
- `GameListPage` — Spielliste/Erstellen/Bearbeiten/Währungsverwaltung
- `WithdrawPage` — Auszahlungsauftragsliste/Prüf-Button/Schalter/Limit-Einstellungen
- `PlatformUserPage` — C端-Benutzerliste/Details/Sperren
- `PaymentPage` — Zahlungsmethodenverwaltung
- `AnnouncementPage` — Ankündigungsverwaltung
- `DashboardPage` — Plattform-Dashboard-Panel erweitern
- Export-Buttons in den Listenseiten

**platform Flutter (neu, PC-Stil):**
- `LoginPage` — Login/Registrierung
- `GameHallPage` — Spielehalle (Liste/Suche)
- `GameDetailPage` — Spieldetails/Start
- `WalletPage` — Wallet-Guthaben/Transaktionen
- `DepositPage` — Einzahlung
- `ExchangePage` — Umtausch
- `WithdrawPage` — Auszahlung
- `ProfilePage` — Persönlicher Bereich

---

## Selbstprüfung

1. **Spec-Abdeckung:** Jede Spec-Anforderung hat entsprechende Aufgaben:
   - Infrastruktur → Aufgaben 1-5
   - Datenbanktabellen → Aufgabe 6
   - Modelle → Aufgaben 7-9
   - C端-Authentifizierung → Aufgabe 11
   - C端-Wallet → Aufgabe 12
   - C端-Umtausch → Aufgabe 13
   - C端-Einzahlung/Auszahlung → Aufgabe 14
   - C端-Spiel/Benutzer → Aufgabe 15
   - Admin-Spiel-CRUD → Aufgabe 16
   - Admin-Auszahlungsprüfung/Schalter → Aufgabe 17
   - Admin-Plattformnutzer/Zahlungen/Ankündigungen → Aufgabe 18
   - Flutter-Frontend → Aufgaben 21-25 (zusammengefasst)

2. **Platzhalter-Scan:** Keine TBD, TODO oder unvollständigen Code. Alle Aufgaben haben vollständige Codeblöcke.

3. **Typkonsistenz:**
   - Alle IDs sind BIGINT → Snowflake::generate()
   - API-IDs sind hashid-Strings → encodeId/decodeId
   - Geldbeträge verwenden String-Darstellung für Dezimalgenauigkeit → bcmath
   - Wallet-Operationen verwenden Optimistic Locking → version-Feld
   - JWT-Authentifizierung → jwt()->create/verify
   - Modell-Namespace → common\model\
   - Middleware-Namespace → common\middleware\
   - Controller-Namespace → app\api\v1\controller\ (service) und app\admin\controller\ (admin)
