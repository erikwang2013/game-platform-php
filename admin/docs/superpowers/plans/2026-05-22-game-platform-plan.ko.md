...
**Architecture:** admin/ (관리 백엔드)와 service/ (C단 비즈니스)는 독립된 webman v2 인스턴스이며, common/ (PSR-4 autoload)을 통해 Model과 Service를 공유한다. 데이터베이스 마이그레이션은 install/에 통합한다. 프론트엔드는 Flutter PC 관리 백엔드 확장 + 새 C단 PC 플랫폼 신설.
