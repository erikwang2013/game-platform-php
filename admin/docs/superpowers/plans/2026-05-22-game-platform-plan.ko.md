# 글로벌 게임 통합 플랫폼 베이직 에디션 — 구현 계획
<!-- lang-nav -->

Languages: [中文](2026-05-22-game-platform-plan.md) · [English](2026-05-22-game-platform-plan.en.md) · **한국어** · [Русский](2026-05-22-game-platform-plan.ru.md) · [Deutsch](2026-05-22-game-platform-plan.de.md) · [Français](2026-05-22-game-platform-plan.fr.md) · [Español](2026-05-22-game-platform-plan.es.md) · [Português](2026-05-22-game-platform-plan.pt.md) · [हिन्दी](2026-05-22-game-platform-plan.hi.md) · [العربية](2026-05-22-game-platform-plan.ar.md) · [বাংলা](2026-05-22-game-platform-plan.bn.md) · [Bahasa Indonesia](2026-05-22-game-platform-plan.id.md) · [日本語](2026-05-22-game-platform-plan.ja.md)


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 핵심 루프 구동: 회원가입 → 충전 → 환전 → 게임 코인 → 출금 → 관리자 심사

**Architecture:** admin/ (관리 백엔드)와 service/ (C단 비즈니스)는 독립된 webman v2 인스턴스이며, common/ (PSR-4 autoload)을 통해 Model과 Service를 공유한다. 데이터베이스 마이그레이션은 admin/database/migrations/에 통합한다. 프론트엔드는 Flutter PC 관리 백엔드 확장 + 새 C단 PC 플랫폼 신설.

**Tech Stack:** PHP 8.3+, webman v2, MySQL 8.0, Redis, erikwang2013/* 계열 컴포넌트
...
**책임 경계:**
- `common/model/` — Eloquent Model, 데이터 테이블과 1:1 대응, casts/relations 정의
- `service/app/api/v1/controller/` — C단 API, 사용자 요청 처리, Model 호출
- `admin/app/admin/controller/` — 관리 백엔드 API, 관리자 작업
- `common/middleware/UserAuth.php` — C단 JWT 검증 미들웨어, userId 주입

---

## Phase 1: 인프라 구축

### Task 1: common/composer.json 생성 및 네임스페이스 정의
...
### Task 2: common/ 등록 (admin) 및 service/ autoload 설정 생성
...
### Task 3: service/ 디렉터리 구조와 composer.json 구축
...
### Task 5: 공유 미들웨어를 service/로 복사하고 UserAuth 생성
...
### Task 6: 베이직 에디션 12개 테이블 SQL 마이그레이션 생성
...
## Phase 7: 최종 라우트 통합 및 검증

### Task 20: 최종 service/config/route.php 통합
...
## Phase 8: Flutter 프론트엔드 (요약)

### Task 21-25: Flutter PC 관리 백엔드 확장 + C단 플랫폼 신설

**Note:** Full Flutter implementation details to be expanded in a follow-up plan. This section summarizes the pages needed.

**admin Flutter (확장):**
- `GameListPage` — 게임 목록/생성/수정/코인 관리
- `WithdrawPage` — 출금 주문 목록/심사 버튼/스위치/한도 설정
- `PlatformUserPage` — C단 사용자 목록/상세/차단
- `PaymentPage` — 결제 수단 관리
- `AnnouncementPage` — 공지 관리
- `DashboardPage` — 플랫폼 대시보드 패널 확장
- Export buttons in list pages

**platform Flutter (신규 PC 스타일):**
- `LoginPage` — 로그인/회원가입
- `GameHallPage` — 게임 홀 (목록/검색)
- `GameDetailPage` — 게임 상세/시작
- `WalletPage` — 지갑 잔액/거래 내역
- `DepositPage` — 충전
- `ExchangePage` — 환전
- `WithdrawPage` — 출금
- `ProfilePage` — 마이페이지
