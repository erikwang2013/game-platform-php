...
Languages: [中文](2026-05-20-backend-enhancement-plan.md) · **English** · [한국어](2026-05-20-backend-enhancement-plan.ko.md) · [Русский](2026-05-20-backend-enhancement-plan.ru.md) · [Deutsch](2026-05-20-backend-enhancement-plan.de.md) · [Français](2026-05-20-backend-enhancement-plan.fr.md) · [Español](2026-05-20-backend-enhancement-plan.es.md) · [Português](2026-05-20-backend-enhancement-plan.pt.md) · [हिन्दी](2026-05-20-backend-enhancement-plan.hi.md) · [العربية](2026-05-20-backend-enhancement-plan.ar.md) · [বাংলা](2026-05-20-backend-enhancement-plan.bn.md) · [Bahasa Indonesia](2026-05-20-backend-enhancement-plan.id.md) · [日本語](2026-05-20-backend-enhancement-plan.ja.md)


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement 13 backend enhancements: 3 new middleware components (CORS, RateLimit, OperationLog), 6 new controllers, 2 model modifications, route + middleware configuration.

**Architecture:** Follow the existing webman pattern — middleware implements `MiddlewareInterface::process()`, controllers extend `BaseController`, routes are defined with closures or `[class, method]`. Global middleware is registered in `config/middleware.php`, route-level middleware is mounted on route groups in `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (extension), PhpSpreadsheet (import)

**Dependency Order:** Model modifications → middleware → controllers → routes/config

---

### Task 1: Add SoftDeletes + Searchable traits to the AdminUser model

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **Step 1: Modify the AdminUser model**

Insert two `use` imports after line 10 of `app/model/AdminUser.php`, and use the two traits at the very top of the class body:
...
Also add a `toSearchableArray()` method at the end of the class (after the `roles()` method, before the closing brace):
...
- [ ] **Step 1: Create the RateLimit middleware**
...
### Task 4: OperationLog middleware

**Files:**
- Create: `app/middleware/OperationLog.php`

- [ ] **Step 1: Create the OperationLog middleware**
...
### Task 5: Global middleware configuration

**Files:**
- Modify: `config/middleware.php`

- [ ] **Step 1: Register global middleware**

Replace the contents of `config/middleware.php` with:
...
### Task 6: Health check controller
...
### Task 7: System config CRUD controller
...
### Task 8: Operation log query controller
...
### Task 9: Profile controller (including logout)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT blacklist check)

- [ ] **Step 1: Modify the AdminAuth middleware to add a JWT blacklist check**

In the `AdminAuth::process()` method, insert the blacklist check after the token is extracted and before the JWT is decoded:
...
A Redis import is also required. Modify the file header:
...
### Task 10: File upload controller

**Files:**
- Create: `app/admin/controller/UploadController.php`

- [ ] **Step 1: Create UploadController**
...
### Task 11: User batch operations

**Files:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **Step 1: Add two methods at the end of the UserController class (after the `destroy` method)**
...
### Task 12: Excel import controller
...
### Task 13: Route update — all new routes + middleware binding
...
Add new routes inside the `/admin` route group, and append the OperationLog middleware. Full route file:
...
After implementation is complete, verify each item:
