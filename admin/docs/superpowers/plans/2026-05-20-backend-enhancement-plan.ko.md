...
Languages: [中文](2026-05-20-backend-enhancement-plan.md) · [English](2026-05-20-backend-enhancement-plan.en.md) · **한국어** · [Русский](2026-05-20-backend-enhancement-plan.ru.md) · [Deutsch](2026-05-20-backend-enhancement-plan.de.md) · [Français](2026-05-20-backend-enhancement-plan.fr.md) · [Español](2026-05-20-backend-enhancement-plan.es.md) · [Português](2026-05-20-backend-enhancement-plan.pt.md) · [हिन्दी](2026-05-20-backend-enhancement-plan.hi.md) · [العربية](2026-05-20-backend-enhancement-plan.ar.md) · [বাংলা](2026-05-20-backend-enhancement-plan.bn.md) · [Bahasa Indonesia](2026-05-20-backend-enhancement-plan.id.md) · [日本語](2026-05-20-backend-enhancement-plan.ja.md)


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 13가지 백엔드 강화 구현: 3개의 새 미들웨어 (CORS, RateLimit, OperationLog), 6개의 새 컨트롤러, 2개의 모델 수정, 라우트 + 미들웨어 설정.

**Architecture:** 기존 webman 패턴 준수 — 미들웨어는 `MiddlewareInterface::process()`를 구현하고, 컨트롤러는 `BaseController`를 상속하며, 라우트는 클로저 또는 `[class, method]`로 정의한다. 전역 미들웨어는 `config/middleware.php`에 등록하고, 라우트 레벨 미들웨어는 `config/route.php`의 라우트 그룹에 마운트한다.

**Tech Stack:** PHP 8.3+, webman v2, Redis (확장), PhpSpreadsheet (가져오기)

**의존성 순서:** 모델 수정 → 미들웨어 → 컨트롤러 → 라우트/설정

---

### Task 1: AdminUser 모델에 SoftDeletes + Searchable trait 추가

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **Step 1: AdminUser 모델 수정**

`app/model/AdminUser.php` 10번째 줄 뒤에 두 개의 `use` 임포트를 삽입하고, 클래스 본문 맨 앞에서 두 trait을 사용한다:
...
동시에 클래스 끝부분(`roles()` 메서드 뒤, 클래스 닫는 중괄호 앞)에 `toSearchableArray()` 메서드를 추가한다:
...
### Task 3: RateLimit 요청 제한 미들웨어

**Files:**
- Create: `app/middleware/RateLimit.php`

- [ ] **Step 1: RateLimit 미들웨어 생성**
...
### Task 4: OperationLog 작업 로그 미들웨어

**Files:**
- Create: `app/middleware/OperationLog.php`

- [ ] **Step 1: OperationLog 미들웨어 생성**
...
`config/middleware.php` 내용을 다음으로 변경:
...
### Task 7: 시스템 설정 CRUD 컨트롤러

**Files:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **Step 1: ConfigController 생성**
...
### Task 9: 마이페이지 컨트롤러 (로그아웃 포함)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT 블랙리스트 검증)

- [ ] **Step 1: AdminAuth 미들웨어 수정, JWT 블랙리스트 검증 추가**

`AdminAuth::process()` 메서드에서 token 추출 후, JWT 디코딩 전에 블랙리스트 검사를 삽입한다:
...
Redis 임포트도 함께 추가해야 한다. 파일 헤더 수정:
...
- [ ] **Step 1: UserController 클래스 끝에 두 메서드 추가 (`destroy` 메서드 뒤)**
...
### Task 12: Excel 가져오기 컨트롤러

**Files:**
- Create: `app/admin/controller/ImportController.php`

- [ ] **Step 1: ImportController 생성**
...
### Task 13: 라우트 업데이트 — 모든 새 라우트 + 미들웨어 바인딩
...
`/admin` 라우트 그룹 안에 새 라우트를 추가하고 OperationLog 미들웨어를 추가한다. 전체 라우트 파일:
