...
**Architecture:** admin/（管理バックエンド）と service/（C端ビジネス）は独立した webman v2 インスタンスで、common/（PSR-4 autoload）を通じて Model と Service を共有する。データベースマイグレーションは install/ に一元配置する。フロントエンドは Flutter PC 管理バックエンドの拡張 + 新規 C端 PC プラットフォーム。
