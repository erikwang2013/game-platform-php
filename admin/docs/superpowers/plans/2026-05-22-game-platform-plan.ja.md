# グローバルゲームアグリゲーションプラットフォーム基本版 — 実装計画
<!-- lang-nav -->
...
**Goal:** コアのループを完成させる: 登録 → チャージ → 両替 → ゲームコイン → 出金 → 管理画面での審査

**Architecture:** admin/（管理バックエンド）と service/（C端ビジネス）は独立した webman v2 インスタンスで、common/（PSR-4 autoload）を通じて Model と Service を共有する。データベースマイグレーションは admin/database/migrations/ に一元配置する。フロントエンドは Flutter PC 管理バックエンドの拡張 + 新規 C端 PC プラットフォーム。

**Tech Stack:** PHP 8.3+, webman v2, MySQL 8.0, Redis, erikwang2013/* シリーズコンポーネント
...
- `common/model/` — Eloquent Model。データテーブルと一対一で対応し、casts/relations を定義
- `service/app/api/v1/controller/` — C端API。ユーザーリクエストを処理し、Model を呼び出す
- `admin/app/admin/controller/` — 管理バックエンドAPI。管理者の操作
- `common/middleware/UserAuth.php` — C端JWT検証ミドルウェア。userId を注入

---

## Phase 1: インフラ構築

### Task 1: common/composer.json を作成し、ネームスペースを定義
...
### Task 2: common/ を admin に登録し、service/ の autoload 設定を作成
...
### Task 3: service/ ディレクトリ構造と composer.json を構築
...
### Task 5: 共有ミドルウェアを service/ にコピーし、UserAuth を作成
...
### Task 6: 基本版 12 テーブルの SQL マイグレーションを作成
...
### Task 20: 最終的な service/config/route.php 統合
...
## Phase 8: Flutter フロントエンド（概要）

### Task 21-25: Flutter PC 管理バックエンド拡張 + C端プラットフォーム新規作成

**Note:** Full Flutter implementation details to be expanded in a follow-up plan. This section summarizes the pages needed.

**admin Flutter (拡張):**
- `GameListPage` — ゲーム一覧/作成/編集/幣種管理
- `WithdrawPage` — 出金オーダー一覧/審査ボタン/スイッチ/限度額設定
- `PlatformUserPage` — C端ユーザー一覧/詳細/バン
- `PaymentPage` — 支払い方法管理
- `AnnouncementPage` — 公告管理
- `DashboardPage` — プラットフォームダッシュボードパネルを拡張
- Export buttons in list pages

**platform Flutter (新規 PC スタイル):**
- `LoginPage` — ログイン/登録
- `GameHallPage` — ゲームホール（一覧/検索）
- `GameDetailPage` — ゲーム詳細/起動
- `WalletPage` — ウォレット残高/流水
- `DepositPage` — チャージ
- `ExchangePage` — 両替
- `WithdrawPage` — 出金
- `ProfilePage` — プロフィール
