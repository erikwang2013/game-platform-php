...
**Ziel:** 13 Backend-Erweiterungen umsetzen: 3 neue Middlewares (CORS, RateLimit, OperationLog), 6 neue Controller, 2 Modelländerungen, Routing + Middleware-Konfiguration.

**Architektur:** Den bestehenden webman-Mustern folgen — Middlewares implementieren `MiddlewareInterface::process()`, Controller erben von `BaseController`, Routen werden per Closure oder `[class, method]` definiert. Globale Middlewares werden in `config/middleware.php` registriert, routenbezogene Middlewares an den Routengruppen in `config/route.php` montiert.

**Tech-Stack:** PHP 8.3+, webman v2, Redis (Erweiterung), PhpSpreadsheet (Import)

**Abhängigkeitsreihenfolge:** Modelländerungen → Middlewares → Controller → Routing/Konfiguration

---

### Aufgabe 1: SoftDeletes + Searchable-Trait für das AdminUser-Modell
...
Nach Zeile 10 in `app/model/AdminUser.php` zwei `use`-Importe einfügen und am Anfang des Klassenkörpers die beiden Traits verwenden:
...
### Aufgabe 3: RateLimit-Middleware
...
### Aufgabe 4: OperationLog-Middleware
...
- [ ] **Schritt 1: Globale Middlewares registrieren**

Den Inhalt von `config/middleware.php` ändern zu:
...
### Aufgabe 8: Controller für die Abfrage von Betriebsprotokollen
...
### Aufgabe 9: Profil-Controller (inkl. Logout)

**Dateien:**
- Erstellen: `app/admin/controller/ProfileController.php`
- Ändern: `app/middleware/AdminAuth.php` (JWT-Blacklist-Prüfung)

- [ ] **Schritt 1: AdminAuth-Middleware ändern, JWT-Blacklist-Prüfung hinzufügen**
...
Gleichzeitig den Redis-Import hinzufügen. Dateikopf ändern:
...
### Aufgabe 11: Massenoperationen für Benutzer
...
### Aufgabe 13: Routing-Update — alle neuen Routen + Middleware-Bindung

**Dateien:**
- Ändern: `config/route.php`

- [ ] **Schritt 1: Routing-Konfiguration aktualisieren**

Innerhalb der Routengruppe `/admin` neue Routen hinzufügen und zugleich die OperationLog-Middleware anhängen. Vollständige Routendatei:
...
Nach Abschluss der Implementierung Punkt für Punkt verifizieren:
