...
**Architecture:** admin/ (admin backend) and service/ (C-side business) are independent webman v2 instances that share Models and Services through common/ (PSR-4 autoload). Database migrations live together in install/. Frontend: extend the existing Flutter PC admin backend + create a new C-side PC platform.
