...
- `common/model/` — Modelos Eloquent, uno por cada tabla de datos, definen casts/relations
- `service/app/api/v1/controller/` — API del lado C, procesa las solicitudes de los usuarios, llama a los modelos
- `admin/app/admin/controller/` — API del panel de administración, operaciones de administradores
- `common/middleware/UserAuth.php` — middleware de verificación JWT del lado C, inyecta userId

---

## Fase 1: Preparación de la infraestructura

### Tarea 1: Crear common/composer.json y definir el espacio de nombres

**Archivos:**
- Crear: `common/composer.json`

- [ ] **Paso 1: Escribir common/composer.json**
...
- [ ] **Paso 2: Ejecutar composer validate en common/**

Ejecutar: `cd common && composer validate`
Esperado: "valid" (puede advertir sobre la falta de versión, ignorar)
...
### Tarea 2: Registrar common/ en admin y crear la configuración de autoload de service/

**Archivos:**
- Modificar: `admin/composer.json`
- Modificar: `admin/config/autoload.php`

- [ ] **Paso 1: Añadir el repositorio de rutas common al admin/composer.json**

En `admin/composer.json`, añadir al bloque `"autoload"` → `"psr-4"`:

El bloque existente es:
```json
"autoload": {
  "psr-4": {
    "": "./",
...
- [ ] **Paso 2: Ejecutar composer dump-autoload**

Ejecutar: `cd admin && composer dump-autoload`
Esperado: éxito, sin errores

- [ ] **Paso 3: Añadir el autoload de common a admin/config/autoload.php (si es necesario)**

Leer primero el archivo actual. Si no carga common, añadir el mapeo de espacio de nombres.
...
### Tarea 3: Crear la estructura de directorios service/ y composer.json

**Archivos:**
- Crear: `service/composer.json`
- Crear: `service/.env.example`
- Crear: `service/start.php`
- Crear: `service/windows.php`
- Crear: `service/support/bootstrap.php`
- Crear: `service/support/Request.php`
- Crear: `service/support/Response.php`
- Crear: `service/support/Setup.php`
- Crear: `service/app/functions.php`

- [ ] **Paso 1: Escribir service/composer.json**
...
- [ ] **Paso 2: Escribir service/start.php**
...
Espera — en realidad, reconsideremos. service/ debería tener su PROPIO directorio vendor para sus dependencias, pero puede compartir el vendor de admin o instalarse por separado. El enfoque más simple: service/ usa composer install de forma independiente y referencia common/ mediante la configuración de autoload compartida.

Debería simplificar: service/ ejecuta `composer install` de forma independiente. El directorio common/ se referencia mediante el mapeo de autoload PSR-4. Tanto admin como service apuntan a `../common/model` y `../common/middleware`.

En realidad, con webman v2, lo más simple es:
- service/ tiene su propio vendor/ (de composer install)
- Los modelos comunes se cargan automáticamente mediante PSR-4 en composer.json: `"common\\model\\": "../common/model"`
- No se necesita symlink

Escribamos esto correctamente.
...
- [ ] **Paso 3: Escribir service/windows.php** (mismo contenido que el patrón admin/windows.php)
...
- [ ] **Paso 4: Escribir service/.env.example**
...
- [ ] **Paso 5: Escribir service/support/bootstrap.php**
...
(Mínimo — webman detecta automáticamente support/bootstrap.php)

- [ ] **Paso 6: Escribir service/app/functions.php**
...
- [ ] **Paso 7: Copiar los archivos de soporte desde admin**

Ejecutar: `cp admin/support/Request.php service/support/Request.php`
Ejecutar: `cp admin/support/Response.php service/support/Response.php`
Ejecutar: `cp admin/support/Setup.php service/support/Setup.php`

- [ ] **Paso 8: Instalar las dependencias composer de service**

Ejecutar: `cd service && composer install`
Esperado: éxito
...
### Tarea 4: Crear los archivos de configuración de service/
...
- Crear: `service/config/plugin/erikwang2013/hashids/app.php`
- Crear: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Crear: `service/config/plugin/erikwang2013/jwt/app.php`

- [ ] **Paso 1: Copiar las configuraciones base de admin y personalizarlas**

Copiar todos los archivos de configuración de `admin/config/` a `service/config/`, ajustando los valores:

Diferencias clave en service/config/:

`database.php` — misma base de datos, misma conexión
`route.php` — rutas diferentes (/api/* del lado C)
`middleware.php` — cadena de middlewares diferente
`server.php` — puerto diferente (8788 vs 8787)
`snowflake.php` — datacenter_id/worker_id diferentes (datacenter_id=1, worker_id=2)

- [ ] **Paso 2: Escribir service/config/middleware.php**
...
Nota: service necesita sus propias copias de los middlewares Cors, SecurityFilter, RateLimit O referenciar los de admin. Por ahora, hacer copias en service/app/middleware/.

- [ ] **Paso 3: Escribir service/config/route.php** (esqueleto)
...
- [ ] **Paso 4: Copiar las configuraciones de los plugins**

Ejecutar: `cp -r admin/config/plugin service/config/plugin`
...
### Tarea 5: Copiar los middlewares compartidos a service/ y crear UserAuth

**Archivos:**
- Crear: `service/app/middleware/Cors.php`
- Crear: `service/app/middleware/SecurityFilter.php`
- Crear: `service/app/middleware/RateLimit.php`
- Crear: `service/app/middleware/ApiVersion.php`
- Crear: `common/middleware/UserAuth.php`

- [ ] **Paso 1: Copiar los archivos de middleware desde admin**

Ejecutar: `cp admin/app/middleware/Cors.php service/app/middleware/Cors.php`
Ejecutar: `cp admin/app/middleware/SecurityFilter.php service/app/middleware/SecurityFilter.php`
Ejecutar: `cp admin/app/middleware/RateLimit.php service/app/middleware/RateLimit.php`
Ejecutar: `cp admin/app/middleware/ApiVersion.php service/app/middleware/ApiVersion.php`

- [ ] **Paso 2: Actualizar el espacio de nombres en los middlewares copiados**

Cambiar `namespace app\middleware;` a `namespace app\middleware;` (igual, ya que service usa el mismo espacio de nombres).

En realidad, verificar el espacio de nombres — como service/ autocarga `"app\\": "./app"`, el espacio de nombres sigue siendo `app\middleware`.

- [ ] **Paso 3: Crear common/middleware/UserAuth.php**
...
- [ ] **Paso 3: Verificar que existen las tablas**

```bash
mysql -h 127.0.0.1 -u root game_platform -e "SHOW TABLES LIKE 'erik_%';"
```

Esperado: la lista incluye las 12 tablas nuevas + las tablas de admin existentes.
...
## Fase 3: Capa compartida — Modelos

### Tarea 7: Crear common/model/User.php

**Archivos:**
- Crear: `common/model/User.php`

- [ ] **Paso 1: Asegurar que el vendor de common/ está configurado**

Ejecutar: `cd common && composer install 2>/dev/null; echo "OK"`

- [ ] **Paso 2: Escribir common/model/User.php**
...
### Tarea 8: Crear common/model/UserWallet.php (con bloqueo optimista)

**Archivos:**
- Crear: `common/model/UserWallet.php`

- [ ] **Paso 1: Escribir UserWallet.php**
...
### Tarea 9: Crear los 10 modelos restantes

**Archivos:**
- Crear: `common/model/UserGameWallet.php`
- Crear: `common/model/Game.php`
- Crear: `common/model/GameCurrency.php`
- Crear: `common/model/DepositOrder.php`
- Crear: `common/model/WithdrawOrder.php`
- Crear: `common/model/ExchangeRecord.php`
- Crear: `common/model/Transaction.php`
- Crear: `common/model/PaymentMethod.php`
- Crear: `common/model/Announcement.php`
- Crear: `common/model/PlatformConfig.php`

- [ ] **Paso 1: Escribir UserGameWallet.php**
...
- [ ] **Paso 3: Escribir GameCurrency.php**
...
- [ ] **Paso 4: Escribir DepositOrder.php**
...
- [ ] **Paso 5: Escribir WithdrawOrder.php**
...
- [ ] **Paso 6: Escribir ExchangeRecord.php**
...
- [ ] **Paso 7: Escribir Transaction.php**
...
- [ ] **Paso 8: Escribir PaymentMethod.php**
...
- [ ] **Paso 9: Escribir Announcement.php**
...
- [ ] **Paso 10: Escribir PlatformConfig.php**
...
## Fase 4: Controladores API del lado C

### Tarea 10: Crear el BaseController de service y copiar los servicios compartidos

**Archivos:**
- Crear: `service/app/api/v1/controller/BaseController.php`
- Crear: `service/app/common/HashidsService.php`
- Crear: `service/app/common/SnowflakeService.php`
- Crear: `service/app/common/EncryptionService.php`
- Modificar: `service/config/route.php`

- [ ] **Paso 1: Copiar las clases de servicio y adaptar el espacio de nombres**

Ejecutar: `cp admin/app/common/HashidsService.php service/app/common/HashidsService.php`
Ejecutar: `cp admin/app/common/SnowflakeService.php service/app/common/SnowflakeService.php`
Ejecutar: `cp admin/app/common/EncryptionService.php service/app/common/EncryptionService.php`

Actualizar el espacio de nombres: cambiar `app\common\HashidsService` → ya es `app\common`, no se necesita cambio ya que service/ autocarga `app\\` → `./app`.

- [ ] **Paso 2: Escribir BaseController.php**
...
### Tarea 11: Crear AuthController (registro/inicio de sesión/refresco)

**Archivos:**
- Crear: `service/app/api/v1/controller/AuthController.php`
- Modificar: `service/config/route.php`

- [ ] **Paso 1: Escribir AuthController.php**
...
- [ ] **Paso 2: Actualizar service/config/route.php** — añadir las rutas de autenticación:
...
### Tarea 12: Crear WalletController

**Archivos:**
- Crear: `service/app/api/v1/controller/WalletController.php`
- Modificar: `service/config/route.php`

- [ ] **Paso 1: Escribir WalletController.php**
...
- [ ] **Paso 2: Actualizar service/config/route.php** — añadir bajo el grupo autenticado:
...
### Tarea 13: Crear ExchangeController (moneda de plataforma ⇄ moneda de juego)

**Archivos:**
- Crear: `service/app/api/v1/controller/ExchangeController.php`
- Modificar: `service/config/route.php`

- [ ] **Paso 1: Escribir ExchangeController.php**
...
- [ ] **Paso 2: Actualizar route.php — añadir las rutas de intercambio bajo el grupo autenticado**
...
### Tarea 14: Crear DepositController y WithdrawController

**Archivos:**
- Crear: `service/app/api/v1/controller/DepositController.php`
- Crear: `service/app/api/v1/controller/WithdrawController.php`
- Modificar: `service/config/route.php`

- [ ] **Paso 1: Escribir DepositController.php**
...
- [ ] **Paso 2: Escribir WithdrawController.php**
...
- [ ] **Paso 3: Actualizar route.php — añadir las rutas de depósito y retiro**
...
### Tarea 15: Crear GameController y UserController para el lado C

**Archivos:**
- Crear: `service/app/api/v1/controller/GameController.php`
- Crear: `service/app/api/v1/controller/UserController.php`
- Modificar: `service/config/route.php`

- [ ] **Paso 1: Escribir GameController.php**
...
- [ ] **Paso 2: Escribir UserController.php**
...
- [ ] **Paso 3: Actualizar route.php** — añadir las rutas de juegos y usuarios
...
## Fase 5: Extensión del panel de administración

### Tarea 16: Crear el GameController de admin (CRUD)

**Archivos:**
- Crear: `admin/app/admin/controller/GameController.php`
- Modificar: `admin/config/route.php`

- [ ] **Paso 1: Escribir el GameController.php de admin**
...
- [ ] **Paso 2: Actualizar admin/config/route.php** — añadir las rutas de juegos en el grupo /admin:
...
### Tarea 17: Crear el WithdrawController de admin (revisión + interruptor + límites)

**Archivos:**
- Crear: `admin/app/admin/controller/WithdrawController.php`
- Modificar: `admin/config/route.php`

- [ ] **Paso 1: Escribir el WithdrawController.php de admin**
...
- [ ] **Paso 2: Actualizar route.php — añadir las rutas de retiro**
...
### Tarea 18: Crear PlatformUserController, PaymentController, AnnouncementController + extender Dashboard y Export

**Archivos:**
- Crear: `admin/app/admin/controller/PlatformUserController.php`
- Crear: `admin/app/admin/controller/PaymentController.php`
- Crear: `admin/app/admin/controller/AnnouncementController.php`
- Modificar: `admin/app/admin/controller/DashboardController.php`
- Modificar: `admin/app/admin/controller/ExportController.php`
- Modificar: `admin/config/route.php`

- [ ] **Paso 1: Escribir PlatformUserController.php**
...
- [ ] **Paso 2: Escribir PaymentController.php** (mínimo para MVP — listar métodos + alternar)
...
- [ ] **Paso 3: Escribir AnnouncementController.php**
...
- [ ] **Paso 4: Extender DashboardController — añadir el método platformDashboard**
...
- [ ] **Paso 5: Extender ExportController — añadir exportaciones de usuarios y transacciones**
...
- [ ] **Paso 6: Actualizar route.php — añadir todas las rutas nuevas de admin**
...
## Fase 6: Añadir la interfaz C端 de anuncios

### Tarea 19: Crear el AnnouncementController del lado C

**Archivos:**
- Crear: `service/app/api/v1/controller/AnnouncementController.php`
- Modificar: `service/config/route.php`

- [ ] **Paso 1: Escribir AnnouncementController.php**
...
## Fase 7: Integración final de rutas y verificación

### Tarea 20: Integración final de service/config/route.php

- [ ] **Paso 1: Escribir el service/config/route.php completo**
...
- [ ] **Paso 2: Verificar que service/ puede arrancar**
...
- [ ] **Paso 3: Probar un endpoint público**
...
## Fase 8: Frontend Flutter (resumen)

### Tareas 21-25: Extensión del panel de administración PC en Flutter + nueva plataforma C端

**Nota:** Los detalles completos de implementación de Flutter se ampliarán en un plan posterior. Esta sección resume las páginas necesarias.

**admin Flutter (extensión):**
- `GameListPage` — lista de juegos/crear/editar/gestión de monedas
- `WithdrawPage` — lista de órdenes de retiro/botón de revisión/interruptor/ajuste de límites
- `PlatformUserPage` — lista de usuarios C端/detalles/bloqueo
- `PaymentPage` — gestión de métodos de pago
- `AnnouncementPage` — gestión de anuncios
- `DashboardPage` — extensión del panel de la plataforma
- Botones de exportación en las páginas de lista

**platform Flutter (nueva, estilo PC):**
- `LoginPage` — inicio de sesión/registro
- `GameHallPage` — sala de juegos (lista/búsqueda)
- `GameDetailPage` — detalles del juego/iniciar
- `WalletPage` — saldo de la cartera/flujo de transacciones
- `DepositPage` — recarga
- `ExchangePage` — intercambio
- `WithdrawPage` — retiro
- `ProfilePage` — centro personal

---

## Auto-revisión

1. **Cobertura de la especificación:** Cada requisito de la especificación tiene sus tareas correspondientes:
   - Infraestructura → Tareas 1-5
   - Tablas de base de datos → Tarea 6
   - Modelos → Tareas 7-9
   - Auth C端 → Tarea 11
   - Wallet C端 → Tarea 12
   - Exchange C端 → Tarea 13
   - Deposit/Withdraw C端 → Tarea 14
   - Game/User C端 → Tarea 15
   - CRUD de juegos en admin → Tarea 16
   - Revisión/interruptor de retiros en admin → Tarea 17
   - Usuarios de plataforma/pagos/anuncios en admin → Tarea 18
   - Frontend Flutter → Tareas 21-25 (resumidas)

2. **Escaneo de marcadores de posición:** Sin TBD, TODO ni código incompleto. Todas las tareas tienen bloques de código completos.

3. **Consistencia de tipos:**
   - Todos los IDs son BIGINT → Snowflake::generate()
   - Los IDs de API son cadenas hashid → encodeId/decodeId
   - Los montos monetarios usan representación de cadena para precisión decimal → bcmath
   - Las operaciones de cartera usan bloqueo optimista → campo version
   - Autenticación JWT → jwt()->create/verify
   - Espacio de nombres de modelos → common\model\
   - Espacio de nombres de middlewares → common\middleware\
   - Espacio de nombres de controladores → app\api\v1\controller\ (service) y app\admin\controller\ (admin)
