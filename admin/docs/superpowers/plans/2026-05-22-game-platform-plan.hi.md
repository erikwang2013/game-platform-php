# वैश्विक गेम एग्रीगेशन प्लेटफ़ॉर्म मूल संस्करण — कार्यान्वयन योजना
<!-- lang-nav -->

Languages: [中文](2026-05-22-game-platform-plan.md) · [English](2026-05-22-game-platform-plan.en.md) · [한국어](2026-05-22-game-platform-plan.ko.md) · [Русский](2026-05-22-game-platform-plan.ru.md) · [Deutsch](2026-05-22-game-platform-plan.de.md) · [Français](2026-05-22-game-platform-plan.fr.md) · [Español](2026-05-22-game-platform-plan.es.md) · [Português](2026-05-22-game-platform-plan.pt.md) · **हिन्दी** · [العربية](2026-05-22-game-platform-plan.ar.md) · [বাংলা](2026-05-22-game-platform-plan.bn.md) · [Bahasa Indonesia](2026-05-22-game-platform-plan.id.md) · [日本語](2026-05-22-game-platform-plan.ja.md)


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**लक्ष्य:** मुख्य क्लोज्ड लूप चलाएँ: पंजीकरण → रिचार्ज → विनिमय → गेम सिक्के → निकासी → बैकएंड समीक्षा

**आर्किटेक्चर:** admin/ (प्रबंधन बैकएंड) और service/ (C-एंड व्यवसाय) स्वतंत्र webman v2 इंस्टेंस हैं, जो common/ (PSR-4 autoload) के माध्यम से Model और Service साझा करते हैं। डेटाबेस माइग्रेशन एकीकृत रूप से `admin/database/migrations/` में रखे जाते हैं। फ्रंटएंड Flutter PC प्रबंधन बैकएंड विस्तार + नया C-एंड PC प्लेटफ़ॉर्म।

**टेक स्टैक:** PHP 8.3+, webman v2, MySQL 8.0, Redis, erikwang2013/* श्रृंखला के कंपोनेंट
...
**जिम्मेदारी सीमाएँ:**
- `common/model/` — Eloquent Model, डेटा टेबल से एक-से-एक संबंधित, casts/relations परिभाषित करते हैं
- `service/app/api/v1/controller/` — C-एंड API, उपयोगकर्ता अनुरोधों को संभालता है, Model को कॉल करता है
- `admin/app/admin/controller/` — प्रबंधन बैकएंड API, व्यवस्थापक संचालन
- `common/middleware/UserAuth.php` — C-एंड JWT सत्यापन मिडलवेयर, userId इंजेक्ट करता है

---

## Phase 1: बुनियादी ढाँचा निर्माण

### Task 1: common/composer.json बनाएँ और नेमस्पेस परिभाषित करें
...
### Task 2: common/ को admin में पंजीकृत करें और service/ autoload कॉन्फ़िगरेशन बनाएँ
...
### Task 3: service/ निर्देशिका संरचना और composer.json बनाएँ
...
रुकिए — वास्तव में, फिर से सोचें। service/ के पास अपनी निर्भरताओं के लिए अपनी स्वयं की vendor निर्देशिका होनी चाहिए, लेकिन यह admin की vendor साझा कर सकता है या अलग से इंस्टॉल कर सकता है। सबसे सरल दृष्टिकोण: service/ स्वतंत्र रूप से composer install का उपयोग करता है, और common/ को साझा autoload कॉन्फ़िगरेशन द्वारा संदर्भित करता है।

मैं सरलीकरण करूँगा: service/ स्वतंत्र रूप से `composer install` करता है। common/ निर्देशिका को PSR-4 autoload मैपिंग के माध्यम से संदर्भित किया जाता है। admin और service दोनों `../common/model` और `../common/middleware` की ओर इंगित करते हैं।

वास्तव में, webman v2 के साथ, सबसे सरल तरीका यह है:
- service/ के पास अपनी स्वयं की vendor/ है (composer install से)
- सामान्य मॉडल composer.json में PSR-4 autoload के माध्यम से लोड होते हैं: `"common\\model\\": "../common/model"`
- कोई symlink आवश्यक नहीं

आइए इसे सही ढंग से लिखें।
...
### Task 4: service/ कॉन्फ़िगरेशन फ़ाइलें बनाएँ
...
नोट: service को Cors, SecurityFilter, RateLimit मिडलवेयर की अपनी प्रतियाँ चाहिए या admin की संदर्भ लेना चाहिए। फिलहाल, service/app/middleware/ में प्रतियाँ बनाएँ।
...
### Task 5: साझा मिडलवेयर को service/ में कॉपी करें और UserAuth बनाएँ
...
वास्तव में, नेमस्पेस सत्यापित करें — चूँकि service `"app\\": "./app"` autoload करता है, नेमस्पेस `app\middleware` ही रहता है।
...
## Phase 2: डेटाबेस माइग्रेशन

### Task 6: मूल संस्करण की 12 टेबलों के लिए SQL माइग्रेशन बनाएँ
...
## Phase 5: प्रबंधन बैकएंड विस्तार
...
## Phase 6: C端 घोषणा इंटरफ़ेस जोड़ें
...
## Phase 7: अंतिम रूट एकीकरण और सत्यापन

### Task 20: अंतिम service/config/route.php एकीकरण
...
## Phase 8: Flutter फ्रंटएंड (सारांश)

### Task 21-25: Flutter PC प्रबंधन बैकएंड विस्तार + C端 प्लेटफ़ॉर्म निर्माण

**नोट:** पूर्ण Flutter कार्यान्वयन विवरण एक अनुवर्ती योजना में विस्तारित किया जाएगा। यह खंड आवश्यक पेजों का सारांश देता है।

**admin Flutter (विस्तार):**
