# Plataforma Global de Agregação de Jogos (Versão Básica) — Plano de Implementação
<!-- lang-nav -->

Languages: [中文](2026-05-22-game-platform-plan.md) · [English](2026-05-22-game-platform-plan.en.md) · [한국어](2026-05-22-game-platform-plan.ko.md) · [Русский](2026-05-22-game-platform-plan.ru.md) · [Deutsch](2026-05-22-game-platform-plan.de.md) · [Français](2026-05-22-game-platform-plan.fr.md) · [Español](2026-05-22-game-platform-plan.es.md) · **Português** · [हिन्दी](2026-05-22-game-platform-plan.hi.md) · [العربية](2026-05-22-game-platform-plan.ar.md) · [বাংলা](2026-05-22-game-platform-plan.bn.md) · [Bahasa Indonesia](2026-05-22-game-platform-plan.id.md) · [日本語](2026-05-22-game-platform-plan.ja.md)


> **Para agentes de trabalho:** SUB-SKILL OBRIGATÓRIO: Use superpowers:subagent-driven-development (recomendado) ou superpowers:executing-plans para implementar este plano tarefa por tarefa. As etapas usam a sintaxe de checkbox (`- [ ]`) para rastreamento.

**Objetivo:** Fazer funcionar o ciclo principal: registro → depósito → troca → moeda do jogo → saque → revisão no painel administrativo

**Arquitetura:** admin/ (painel administrativo) e service/ (negócio do lado do cliente C) são instâncias webman v2 independentes, compartilhando Model e Service por meio de common/ (autoload PSR-4). As migrações de banco de dados ficam unificadas em admin/database/migrations/. Frontend: extensão do painel administrativo PC Flutter + criação da nova plataforma PC do lado C.

**Stack de Tecnologias:** PHP 8.3+, webman v2, MySQL 8.0, Redis, componentes da série erikwang2013/*
...
│   ├── composer.json
│   ├── model/
│   │   ├── User.php                    # Usuário do lado C
│   │   ├── UserWallet.php              # Carteira de moedas da plataforma (com lock otimista)
│   │   ├── UserGameWallet.php          # Carteira de moedas do jogo
│   │   ├── Game.php                    # Jogo
│   │   ├── GameCurrency.php            # Moeda do jogo
│   │   ├── DepositOrder.php            # Pedido de recarga
│   │   ├── WithdrawOrder.php           # Pedido de saque
│   │   ├── ExchangeRecord.php          # Registro de troca
│   │   ├── Transaction.php             # Fluxo financeiro da plataforma
│   │   ├── PaymentMethod.php           # Método de pagamento
│   │   ├── Announcement.php            # Anúncio
│   │   └── PlatformConfig.php          # Configuração da plataforma
│   └── middleware/
│       └── UserAuth.php                # Middleware de autenticação JWT do usuário do lado C
│
├── service/
│   ├── composer.json
│   ├── start.php
...
│
├── admin/
│   ├── composer.json                  # Adicionar repositório path common/ + novos controllers
│   ├── app/admin/controller/
│   │   ├── GameController.php         # Novo
│   │   ├── WithdrawController.php     # Novo
│   │   ├── PaymentController.php      # Novo
│   │   ├── PlatformUserController.php # Novo
│   │   ├── AnnouncementController.php # Novo
│   │   ├── DashboardController.php    # Estender painel da plataforma
│   │   └── ExportController.php       # Estender exportação de fluxo/saque
│   ├── app/model/ -> ../../common/model/  (symlink, ou autoload)
│   ├── config/route.php               # Estender com novas rotas
│   ├── config/autoload.php            # Estender com o namespace common
│   └── database/migrations/
│       └── 2026_05_22_000003_platform_tables.sql  # 12 tabelas da versão básica
│
└── apps/flutter/
    ├── admin/                          # Painel administrativo PC (estender o existente)
    └── platform/                       # Plataforma de usuário PC lado C (novo)
```

**Limites de responsabilidade:**
- `common/model/` — Model Eloquent, um para cada tabela de dados, define casts/relations
- `service/app/api/v1/controller/` — API do lado C, processa solicitações de usuários, chama Models
- `admin/app/admin/controller/` — API do painel administrativo, operações de administradores
- `common/middleware/UserAuth.php` — Middleware de verificação JWT do lado C, injeta userId

---

## Phase 1: Configuração da infraestrutura

### Task 1: Criar common/composer.json e definir o namespace

**Arquivos:**
- Criar: `common/composer.json`

- [ ] **Step 1: Escrever common/composer.json**
...
- [ ] **Step 2: Executar composer validate em common/**

Executar: `cd common && composer validate`
Esperado: "valid" (pode avisar sobre falta de versão, ignorar)
...
### Task 2: Registrar common/ no admin e criar a configuração de autoload do service/

**Arquivos:**
- Modificar: `admin/composer.json`
- Modificar: `admin/config/autoload.php`

- [ ] **Step 1: Adicionar o repositório path common ao admin/composer.json**

Em `admin/composer.json`, adicione ao bloco `"autoload"` → `"psr-4"`:
...
- [ ] **Step 2: Executar composer dump-autoload**

Executar: `cd admin && composer dump-autoload`
Esperado: sucesso, sem erros

- [ ] **Step 3: Adicionar o autoload common ao admin/config/autoload.php (se necessário)**

Leia o arquivo atual primeiro. Se ele ainda não carrega common, adicione o mapeamento de namespace.
...
### Task 3: Montar a estrutura de diretórios do service/ e composer.json

**Arquivos:**
- Criar: `service/composer.json`
- Criar: `service/.env.example`
- Criar: `service/start.php`
- Criar: `service/windows.php`
- Criar: `service/support/bootstrap.php`
- Criar: `service/support/Request.php`
- Criar: `service/support/Response.php`
- Criar: `service/support/Setup.php`
- Criar: `service/app/functions.php`

- [ ] **Step 1: Escrever service/composer.json**
...
- [ ] **Step 2: Escrever service/start.php**
...
Espere — na verdade, repense. service/ deve ter seu PRÓPRIO diretório vendor para suas próprias dependências, mas pode compartilhar o vendor do admin ou instalar separadamente. A abordagem mais simples: service/ usa composer install de forma independente e referencia common/ pela configuração de autoload compartilhada.

Devo simplificar: service/ executa `composer install` independentemente. O diretório common/ é referenciado pelo mapeamento de autoload PSR-4. Tanto admin quanto service apontam para `../common/model` e `../common/middleware`.

Na verdade, com webman v2, o mais simples é:
- service/ tem seu próprio vendor/ (do composer install)
- Models comuns autoload via PSR-4 no composer.json: `"common\\model\\": "../common/model"`
- Sem necessidade de symlink

Vou escrever isso corretamente.
...
- [ ] **Step 3: Escrever service/windows.php** (mesmo conteúdo do padrão admin/windows.php)
...
- [ ] **Step 4: Escrever service/.env.example**
...
- [ ] **Step 5: Escrever service/support/bootstrap.php**
...
(Mínimo — webman detecta automaticamente support/bootstrap.php)

- [ ] **Step 6: Escrever service/app/functions.php**
...
- [ ] **Step 7: Copiar os arquivos support do admin**

Executar: `cp admin/support/Request.php service/support/Request.php`
Executar: `cp admin/support/Response.php service/support/Response.php`
Executar: `cp admin/support/Setup.php service/support/Setup.php`

- [ ] **Step 8: Instalar as dependências composer do service**

Executar: `cd service && composer install`
Esperado: sucesso
...
### Task 4: Criar os arquivos de configuração do service/
...
- Criar: `service/config/plugin/erikwang2013/hashids/app.php`
- Criar: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Criar: `service/config/plugin/erikwang2013/jwt/app.php`

- [ ] **Step 1: Copiar as configurações base do admin e personalizar**

Copie todos os arquivos de configuração de `admin/config/` para `service/config/`, ajustando os valores:

Principais diferenças em service/config/:

`database.php` — mesmo banco, mesma conexão
`route.php` — rotas diferentes (lado C /api/*)
`middleware.php` — cadeia de middlewares diferente
`server.php` — porta diferente (8788 vs 8787)
`snowflake.php` — datacenter_id/worker_id diferentes (datacenter_id=1, worker_id=2)

- [ ] **Step 2: Escrever service/config/middleware.php**
...
Nota: service precisa de suas próprias cópias dos middlewares Cors, SecurityFilter, RateLimit OU referenciar os do admin. Por enquanto, faça cópias em service/app/middleware/.

- [ ] **Step 3: Escrever service/config/route.php** (esqueleto)
...
- [ ] **Step 4: Copiar as configurações dos plugins**

Executar: `cp -r admin/config/plugin service/config/plugin`
...
### Task 5: Copiar os middlewares compartilhados para service/ e criar UserAuth

**Arquivos:**
- Criar: `service/app/middleware/Cors.php`
- Criar: `service/app/middleware/SecurityFilter.php`
- Criar: `service/app/middleware/RateLimit.php`
- Criar: `service/app/middleware/ApiVersion.php`
- Criar: `common/middleware/UserAuth.php`

- [ ] **Step 1: Copiar os arquivos de middleware do admin**

Executar: `cp admin/app/middleware/Cors.php service/app/middleware/Cors.php`
Executar: `cp admin/app/middleware/SecurityFilter.php service/app/middleware/SecurityFilter.php`
Executar: `cp admin/app/middleware/RateLimit.php service/app/middleware/RateLimit.php`
Executar: `cp admin/app/middleware/ApiVersion.php service/app/middleware/ApiVersion.php`

- [ ] **Step 2: Atualizar o namespace nos middlewares copiados**

Altere `namespace app\middleware;` para `namespace app\middleware;` (igual, pois o service usa o mesmo namespace).

Na verdade, verifique o namespace — como service/ autoload `"app\\": "./app"`, o namespace permanece `app\middleware`.

- [ ] **Step 3: Criar common/middleware/UserAuth.php**
...
## Phase 2: Migrações de banco de dados

### Task 6: Criar a migração SQL das 12 tabelas da versão básica

**Arquivos:**
- Criar: `admin/database/migrations/2026_05_22_000003_platform_tables.sql`

- [ ] **Step 1: Escrever a migração SQL completa**
...
- [ ] **Step 2: Executar a migração no banco de desenvolvimento**

```bash
mysql -h 127.0.0.1 -u root game_platform < admin/database/migrations/2026_05_22_000003_platform_tables.sql
```

Esperado: sem erros, todas as 12 tabelas criadas.

- [ ] **Step 3: Verificar se as tabelas existem**

```bash
mysql -h 127.0.0.1 -u root game_platform -e "SHOW TABLES LIKE 'erik_%';"
```

Esperado: a lista inclui todas as 12 novas tabelas + as tabelas admin existentes.
...
## Phase 3: Camada compartilhada — Models

### Task 7: Criar common/model/User.php

**Arquivos:**
- Criar: `common/model/User.php`

- [ ] **Step 1: Garantir que o vendor de common/ está configurado**

Executar: `cd common && composer install 2>/dev/null; echo "OK"`

- [ ] **Step 2: Escrever common/model/User.php**
...
### Task 8: Criar common/model/UserWallet.php (com lock otimista)

**Arquivos:**
- Criar: `common/model/UserWallet.php`

- [ ] **Step 1: Escrever UserWallet.php**
...
### Task 9: Criar os 10 models restantes

**Arquivos:**
- Criar: `common/model/UserGameWallet.php`
- Criar: `common/model/Game.php`
- Criar: `common/model/GameCurrency.php`
- Criar: `common/model/DepositOrder.php`
- Criar: `common/model/WithdrawOrder.php`
- Criar: `common/model/ExchangeRecord.php`
- Criar: `common/model/Transaction.php`
- Criar: `common/model/PaymentMethod.php`
- Criar: `common/model/Announcement.php`
- Criar: `common/model/PlatformConfig.php`

- [ ] **Step 1: Escrever UserGameWallet.php**
...
- [ ] **Step 3: Escrever GameCurrency.php**
...
- [ ] **Step 4: Escrever DepositOrder.php**
...
- [ ] **Step 5: Escrever WithdrawOrder.php**
...
- [ ] **Step 6: Escrever ExchangeRecord.php**
...
- [ ] **Step 7: Escrever Transaction.php**
...
- [ ] **Step 8: Escrever PaymentMethod.php**
...
- [ ] **Step 9: Escrever Announcement.php**
...
- [ ] **Step 10: Escrever PlatformConfig.php**
...
## Phase 4: Controllers da API do lado C

### Task 10: Criar o BaseController do service e copiar os serviços compartilhados

**Arquivos:**
- Criar: `service/app/api/v1/controller/BaseController.php`
- Criar: `service/app/common/HashidsService.php`
- Criar: `service/app/common/SnowflakeService.php`
- Criar: `service/app/common/EncryptionService.php`
- Modificar: `service/config/route.php`

- [ ] **Step 1: Copiar as classes de serviço e adaptar o namespace**

Executar: `cp admin/app/common/HashidsService.php service/app/common/HashidsService.php`
Executar: `cp admin/app/common/SnowflakeService.php service/app/common/SnowflakeService.php`
Executar: `cp admin/app/common/EncryptionService.php service/app/common/EncryptionService.php`

Atualize o namespace: altere `app\common\HashidsService` → já é `app\common`, nenhuma alteração necessária, pois service/ autoload `app\\` → `./app`.

- [ ] **Step 2: Escrever BaseController.php**
...
### Task 11: Criar AuthController (register/login/refresh)

**Arquivos:**
- Criar: `service/app/api/v1/controller/AuthController.php`
- Modificar: `service/config/route.php`

- [ ] **Step 1: Escrever AuthController.php**
...
- [ ] **Step 2: Atualizar service/config/route.php** — adicionar rotas de autenticação:
...
### Task 12: Criar WalletController

**Arquivos:**
- Criar: `service/app/api/v1/controller/WalletController.php`
- Modificar: `service/config/route.php`

- [ ] **Step 1: Escrever WalletController.php**
...
- [ ] **Step 2: Atualizar service/config/route.php** — adicionar no grupo autenticado:
...
### Task 13: Criar ExchangeController (moeda da plataforma ⇄ moeda do jogo)

**Arquivos:**
- Criar: `service/app/api/v1/controller/ExchangeController.php`
- Modificar: `service/config/route.php`

- [ ] **Step 1: Escrever ExchangeController.php**
...
- [ ] **Step 2: Atualizar route.php — adicionar as rotas de troca no grupo autenticado**
...
### Task 14: Criar DepositController e WithdrawController

**Arquivos:**
- Criar: `service/app/api/v1/controller/DepositController.php`
- Criar: `service/app/api/v1/controller/WithdrawController.php`
- Modificar: `service/config/route.php`

- [ ] **Step 1: Escrever DepositController.php**
...
- [ ] **Step 2: Escrever WithdrawController.php**
...
- [ ] **Step 3: Atualizar route.php — adicionar as rotas de depósito e saque**
...
### Task 15: Criar GameController e UserController para o lado C

**Arquivos:**
- Criar: `service/app/api/v1/controller/GameController.php`
- Criar: `service/app/api/v1/controller/UserController.php`
- Modificar: `service/config/route.php`

- [ ] **Step 1: Escrever GameController.php**
...
- [ ] **Step 2: Escrever UserController.php**
...
- [ ] **Step 3: Atualizar route.php** — adicionar as rotas de jogo e usuário
...
## Phase 5: Extensão do painel administrativo

### Task 16: Criar o GameController admin (CRUD)

**Arquivos:**
- Criar: `admin/app/admin/controller/GameController.php`
- Modificar: `admin/config/route.php`

- [ ] **Step 1: Escrever admin GameController.php**
...
- [ ] **Step 2: Atualizar admin/config/route.php** — adicionar as rotas de jogo no grupo /admin:
...
### Task 17: Criar o WithdrawController admin (revisão + interruptor + limites)

**Arquivos:**
- Criar: `admin/app/admin/controller/WithdrawController.php`
- Modificar: `admin/config/route.php`

- [ ] **Step 1: Escrever admin WithdrawController.php**
...
- [ ] **Step 2: Atualizar route.php — adicionar as rotas de saque**
...
### Task 18: Criar PlatformUserController, PaymentController, AnnouncementController + estender Dashboard e Export

**Arquivos:**
- Criar: `admin/app/admin/controller/PlatformUserController.php`
- Criar: `admin/app/admin/controller/PaymentController.php`
- Criar: `admin/app/admin/controller/AnnouncementController.php`
- Modificar: `admin/app/admin/controller/DashboardController.php`
- Modificar: `admin/app/admin/controller/ExportController.php`
- Modificar: `admin/config/route.php`

- [ ] **Step 1: Escrever PlatformUserController.php**
...
- [ ] **Step 2: Escrever PaymentController.php** (mínimo para MVP — listar métodos + alternar)
...
- [ ] **Step 3: Escrever AnnouncementController.php**
...
- [ ] **Step 4: Estender DashboardController — adicionar o método platformDashboard**
...
- [ ] **Step 5: Estender ExportController — adicionar exportações de usuários e transações**
...
- [ ] **Step 6: Atualizar route.php** — adicionar todas as novas rotas admin
...
## Phase 6: Adicionar a API de anúncios do lado C

### Task 19: Criar o AnnouncementController do lado C

**Arquivos:**
- Criar: `service/app/api/v1/controller/AnnouncementController.php`
- Modificar: `service/config/route.php`

- [ ] **Step 1: Escrever AnnouncementController.php**
...
## Phase 7: Integração final das rotas e validação

### Task 20: Integração final do service/config/route.php

- [ ] **Step 1: Escrever o service/config/route.php completo**
...
- [ ] **Step 2: Verificar se o service/ consegue iniciar**

```bash
cd service && php start.php start -d
# Verificar: php start.php status
```

- [ ] **Step 3: Testar um endpoint público**

```bash
curl -s http://localhost:8788/health | python3 -m json.tool
```

Esperado: `{"code": 0, "message": "ok"}`
...
## Phase 8: Frontend Flutter (resumo)

### Task 21-25: Extensão do painel administrativo Flutter PC + criação da plataforma lado C

**Nota:** Os detalhes completos da implementação Flutter serão expandidos em um plano complementar. Esta seção resume as páginas necessárias.

**Flutter admin (extensão):**
- `GameListPage` — Lista/criação/edição de jogos/gerenciamento de moedas
- `WithdrawPage` — Lista de pedidos de saque/botões de revisão/interruptor/configuração de limites
- `PlatformUserPage` — Lista de usuários do lado C/detalhes/banimento
- `PaymentPage` — Gerenciamento de métodos de pagamento
- `AnnouncementPage` — Gerenciamento de anúncios
- `DashboardPage` — Estender o painel da plataforma
- Botões de exportação nas páginas de lista

**Flutter platform (novo, estilo PC):**
- `LoginPage` — Login/registro
- `GameHallPage` — Salão de jogos (lista/busca)
- `GameDetailPage` — Detalhes do jogo/início
- `WalletPage` — Saldo da carteira/fluxo
- `DepositPage` — Recarga
- `ExchangePage` — Troca
- `WithdrawPage` — Saque
- `ProfilePage` — Central do usuário

---

## Auto-Revisão

1. **Cobertura da especificação:** Cada requisito da especificação tem tarefas correspondentes:
   - Infraestrutura → Tasks 1-5
   - Tabelas de banco de dados → Task 6
   - Models → Tasks 7-9
   - Auth lado C → Task 11
   - Wallet lado C → Task 12
   - Exchange lado C → Task 13
   - Deposit/Withdraw lado C → Task 14
   - Game/User lado C → Task 15
   - CRUD de games admin → Task 16
   - Revisão/interruptor de saque admin → Task 17
   - Usuários da plataforma/pagamentos/anúncios admin → Task 18
   - Frontend Flutter → Tasks 21-25 (resumido)

2. **Varredura de placeholders:** Sem TBD, TODO ou código incompleto. Todas as tarefas têm blocos de código completos.

3. **Consistência de tipos:**
   - Todos os IDs são BIGINT → Snowflake::generate()
   - IDs de API são strings hashid → encodeId/decodeId
   - Valores monetários usam representação string para precisão decimal → bcmath
   - Operações de carteira usam lock otimista → campo version
   - Autenticação JWT → jwt()->create/verify
   - Namespace de Models → common\model\
   - Namespace de middlewares → common\middleware\
   - Namespace de controllers → app\api\v1\controller\ (service) e app\admin\controller\ (admin)
