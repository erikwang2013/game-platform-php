# Глобальная игровая агрегационная платформа (базовая версия) — план реализации
<!-- lang-nav -->
...
- `common/model/` — Eloquent Model, однозначно соответствуют таблицам БД, определяют casts/relations
- `service/app/api/v1/controller/` — C-сторонние API, обрабатывают запросы пользователей, вызывают Model
- `admin/app/admin/controller/` — API админ-панели, операции администратора
- `common/middleware/UserAuth.php` — C-сторонний JWT-мидлвар аутентификации, внедряет userId

---

## Phase 1: Создание инфраструктуры

### Task 1: Создать common/composer.json и определить пространство имён
...
### Task 2: Зарегистрировать common/ в admin и создать autoload-конфигурацию service/
...
### Task 3: Развернуть структуру каталога service/ и composer.json
...
### Task 4: Создать конфигурационные файлы service/
...
`route.php` — different routes (C-сторона /api/*)
...
### Task 5: Скопировать общие мидлвары в service/ и создать UserAuth
...
## Phase 4: C-сторонние API-контроллеры
...
### Task 15: Create GameController, UserController for C-сторона
...
## Phase 5: Расширение админ-панели
...
## Phase 6: Добавление C-стороннего интерфейса объявлений

### Task 19: Create C-сторонний AnnouncementController
...
## Phase 7: Финальная интеграция и проверка маршрутов

### Task 20: Финальная интеграция service/config/route.php
...
## Phase 8: Flutter-фронтенд (кратко)

### Task 21-25: Расширение Flutter PC-админки + создание C-сторонней платформы

**Note:** Full Flutter implementation details to be expanded in a follow-up plan. This section summarizes the pages needed.

**admin Flutter (расширение):**
- `GameListPage` — список игр / создание / редактирование / управление валютами
- `WithdrawPage` — список заявок на вывод / кнопки проверки / переключатели / настройка лимитов
- `PlatformUserPage` — список C-сторонних пользователей / детали / блокировка
- `PaymentPage` — управление способами оплаты
- `AnnouncementPage` — управление объявлениями
- `DashboardPage` — расширенная панель платформы
- Export buttons in list pages

**platform Flutter (новый, в стиле PC):**
- `LoginPage` — вход / регистрация
- `GameHallPage` — игровой зал (список/поиск)
- `GameDetailPage` — детали игры / запуск
- `WalletPage` — баланс кошелька / транзакции
- `DepositPage` — пополнение
- `ExchangePage` — обмен валют
- `WithdrawPage` — вывод средств
...
1. **Spec coverage:** Each spec requirement has corresponding tasks:
   ...
   - Models → Tasks 7-9
   - C-сторона Auth → Task 11
   - C-сторона Wallet → Task 12
   - C-сторона Exchange → Task 13
   - C-сторона Deposit/Withdraw → Task 14
   - C-сторона Game/User → Task 15
