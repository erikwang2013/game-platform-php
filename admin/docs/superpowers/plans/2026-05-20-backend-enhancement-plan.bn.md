...
Languages: [中文](2026-05-20-backend-enhancement-plan.md) · [English](2026-05-20-backend-enhancement-plan.en.md) · [한국어](2026-05-20-backend-enhancement-plan.ko.md) · [Русский](2026-05-20-backend-enhancement-plan.ru.md) · [Deutsch](2026-05-20-backend-enhancement-plan.de.md) · [Français](2026-05-20-backend-enhancement-plan.fr.md) · [Español](2026-05-20-backend-enhancement-plan.es.md) · [Português](2026-05-20-backend-enhancement-plan.pt.md) · [हिन्दी](2026-05-20-backend-enhancement-plan.hi.md) · [العربية](2026-05-20-backend-enhancement-plan.ar.md) · **বাংলা** · [Bahasa Indonesia](2026-05-20-backend-enhancement-plan.id.md) · [日本語](2026-05-20-backend-enhancement-plan.ja.md)


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** ১৩টি ব্যাকএন্ড উন্নয়ন বাস্তবায়ন: ৩টি নতুন মিডলওয়্যার (CORS, RateLimit, OperationLog), ৬টি নতুন কন্ট্রোলার, ২টি মডেল পরিবর্তন, রাউট + মিডলওয়্যার কনফিগারেশন।

**Architecture:** বিদ্যমান webman প্যাটার্ন অনুসরণ — মিডলওয়্যার `MiddlewareInterface::process()` ইমপ্লিমেন্ট করে, কন্ট্রোলার `BaseController` থেকে ইনহেরিট করে, রাউট ক্লোজার বা `[class, method]` দিয়ে সংজ্ঞায়িত হয়। গ্লোবাল মিডলওয়্যার `config/middleware.php`-তে রেজিস্টার হয়, রাউট-লেভেল মিডলওয়্যার `config/route.php` রাউট গ্রুপে মাউন্ট হয়।

**Tech Stack:** PHP 8.3+, webman v2, Redis (extension), PhpSpreadsheet (import)

**依赖顺序:** মডেল পরিবর্তন → মিডলওয়্যার → কন্ট্রোলার → রাউট/কনফিগারেশন

---

### Task 1: AdminUser 模型中 SoftDeletes + Searchable trait যোগ করা

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **Step 1: AdminUser মডেল পরিবর্তন**

`app/model/AdminUser.php`-এর ১০ নম্বর লাইনের পরে দুটি `use` ইমপোর্ট ঢোকান এবং ক্লাস বডির একদম শুরুতে দুটি trait ব্যবহার করুন:
...
একই সাথে ক্লাসের শেষে (`roles()` মেথডের পরে, ক্লাসের সমাপ্তি বন্ধনীর আগে) `toSearchableArray()` মেথড যোগ করুন:
...
### Task 2: CORS মিডলওয়্যার

**Files:**
- Create: `app/middleware/Cors.php`

- [ ] **Step 1: Cors মিডলওয়্যার তৈরি**
...
### Task 3: RateLimit রেট লিমিট মিডলওয়্যার

**Files:**
- Create: `app/middleware/RateLimit.php`

- [ ] **Step 1: RateLimit মিডলওয়্যার তৈরি**
...
### Task 4: OperationLog অপারেশন লগ মিডলওয়্যার

**Files:**
- Create: `app/middleware/OperationLog.php`

- [ ] **Step 1: OperationLog মিডলওয়্যার তৈরি**
...
### Task 5: গ্লোবাল মিডলওয়্যার কনফিগারেশন

**Files:**
- Modify: `config/middleware.php`

- [ ] **Step 1: গ্লোবাল মিডলওয়্যার রেজিস্টার**

`config/middleware.php`-এর বিষয়বস্তু পরিবর্তন করুন:
...
### Task 6: হেলথ চেক কন্ট্রোলার

**Files:**
- Create: `app/admin/controller/HealthController.php`

- [ ] **Step 1: HealthController তৈরি**
...
### Task 7: সিস্টেম কনফিগ CRUD কন্ট্রোলার

**Files:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **Step 1: ConfigController তৈরি**
...
### Task 8: অপারেশন লগ কুয়েরি কন্ট্রোলার

**Files:**
- Create: `app/admin/controller/LogController.php`

- [ ] **Step 1: LogController তৈরি**
...
### Task 9: প্রোফাইল কন্ট্রোলার (লগআউট সহ)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT ব্ল্যাকলিস্ট যাচাই)

- [ ] **Step 1: AdminAuth মিডলওয়্যার পরিবর্তন — JWT ব্ল্যাকলিস্ট যাচাই যোগ**

`AdminAuth::process()` মেথডে, token এক্সট্রাকশনের পরে এবং JWT ডিকোডের আগে ব্ল্যাকলিস্ট চেক ঢোকান:
...
একই সাথে Redis ইমপোর্ট যোগ করতে হবে। ফাইলের হেডার পরিবর্তন করুন:
...
### Task 10: ফাইল আপলোড কন্ট্রোলার

**Files:**
- Create: `app/admin/controller/UploadController.php`

- [ ] **Step 1: UploadController তৈরি**
...
### Task 11: ইউজার ব্যাচ অপারেশন

**Files:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **Step 1: UserController ক্লাসের শেষে দুটি মেথড যোগ করুন (`destroy` মেথডের পরে)**
...
### Task 12: Excel ইমপোর্ট কন্ট্রোলার

**Files:**
- Create: `app/admin/controller/ImportController.php`

- [ ] **Step 1: ImportController তৈরি**
...
### Task 13: রাউট আপডেট — সমস্ত নতুন রাউট + মিডলওয়্যার বাইন্ডিং

**Files:**
- Modify: `config/route.php`

- [ ] **Step 1: রাউট কনফিগারেশন আপডেট**

`/admin` রাউট গ্রুপের ভেতরে নতুন রাউট যোগ করুন, সাথে OperationLog মিডলওয়্যার যুক্ত করুন। সম্পূর্ণ রাউট ফাইল:
...
বাস্তবায়ন শেষে, প্রতিটি আইটেম যাচাই করুন:
