...
**Goal:** تنفيذ 13 تحسينًا للخلفية: 3 وسائط جديدة (CORS, RateLimit, OperationLog) و6 وحدات تحكم جديدة وتعديل نموذجين وتكوين المسارات + الوسائط.

**Architecture:** اتباع نمط webman الحالي — الوسائط تنفّذ `MiddlewareInterface::process()`، ووحدات التحكم ترث من `BaseController`، والمسارات تُعرَّف بإغلاقات أو `[class, method]`. الوسائط العامة تُسجَّل في `config/middleware.php`، ووسائط مستوى المسار تُركَّب على مجموعات المسارات في `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (امتداد), PhpSpreadsheet (استيراد)

**ترتيب التبعيات:** تعديل النماذج → الوسائط → وحدات التحكم → المسارات/الإعدادات

---

### Task 1: إضافة SoftDeletes و Searchable trait إلى نموذج AdminUser

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **الخطوة 1: تعديل نموذج AdminUser**

إدراج استيرادين `use` بعد السطر 10 من `app/model/AdminUser.php`، واستخدام الـ trait داخل جسم الفئة في المقدمة:
...
وفي نهاية الفئة (بعد طريقة `roles()`، قبل قوس الإغلاق) أضف طريقة `toSearchableArray()`:
...
### Task 3: وسيطة تقييد المعدل RateLimit

**Files:**
- Create: `app/middleware/RateLimit.php`

- [ ] **الخطوة 1: إنشاء وسيطة RateLimit**
...
### Task 4: وسيطة سجل العمليات OperationLog

**Files:**
- Create: `app/middleware/OperationLog.php`

- [ ] **الخطوة 1: إنشاء وسيطة OperationLog**
...
### Task 5: تكوين الوسائط العامة

**Files:**
- Modify: `config/middleware.php`

- [ ] **الخطوة 1: تسجيل الوسائط العامة**

تغيير محتوى `config/middleware.php` إلى:
...
### Task 6: وحدة تحكم فحص الصحة

**Files:**
- Create: `app/admin/controller/HealthController.php`

- [ ] **الخطوة 1: إنشاء HealthController**
...
### Task 7: وحدة تحكم CRUD إعدادات النظام

**Files:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **الخطوة 1: إنشاء ConfigController**
...
### Task 8: وحدة تحكم الاستعلام عن سجلات العمليات

**Files:**
- Create: `app/admin/controller/LogController.php`

- [ ] **الخطوة 1: إنشاء LogController**
...
### Task 9: وحدة تحكم الملف الشخصي (مع تسجيل الخروج)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (التحقق من قائمة JWT السوداء)

- [ ] **الخطوة 1: تعديل وسيطة AdminAuth وإضافة التحقق من قائمة JWT السوداء**

في طريقة `AdminAuth::process()`، بعد استخراج token وقبل فك تشفير JWT، أدرج فحص القائمة السوداء:
...
يلزم إضافة استيراد Redis في نفس الوقت. تعديل رأس الملف:
...
- [ ] **الخطوة 2: إنشاء ProfileController**
...
### Task 10: وحدة تحكم رفع الملفات

**Files:**
- Create: `app/admin/controller/UploadController.php`

- [ ] **الخطوة 1: إنشاء UploadController**
...
### Task 11: العمليات الجماعية على المستخدمين

**Files:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **الخطوة 1: إضافة طريقتين في نهاية فئة UserController (بعد طريقة `destroy`)**
...
### Task 12: وحدة تحكم استيراد Excel

**Files:**
- Create: `app/admin/controller/ImportController.php`

- [ ] **الخطوة 1: إنشاء ImportController**
...
### Task 13: تحديث المسارات — جميع المسارات الجديدة + ربط الوسائط

**Files:**
- Modify: `config/route.php`

- [ ] **الخطوة 1: تحديث تكوين المسارات**

إضافة المسارات الجديدة داخل مجموعة `/admin` مع إلحاق وسيطة OperationLog. ملف المسارات الكامل:
...
بعد اكتمال التنفيذ، تحقق بندًا ببند:

```bash
# 1. فحص صياغة جميع الملفات
find app -name "*.php" -newer app/model/AdminUser.php -exec php -l {} \;
php -l config/route.php
php -l config/middleware.php

# 2. تشغيل الخدمة للتحقق
