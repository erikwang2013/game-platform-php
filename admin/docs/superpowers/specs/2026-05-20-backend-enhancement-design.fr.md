...
La table `erik_operation_log` n'a qu'une colonne `created_at` (pas de `updated_at`). Le `save()` par défaut d'Eloquent tenterait d'écrire `updated_at`, provoquant une erreur SQL.
