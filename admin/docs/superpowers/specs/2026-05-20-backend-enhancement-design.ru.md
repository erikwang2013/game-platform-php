...
Таблица `game_operation_log` содержит только колонку `created_at` (нет `updated_at`). Eloquent при `save()` по умолчанию пытается записать `updated_at`, что вызывает SQL-ошибку.
