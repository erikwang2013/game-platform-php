...
La tabla `game_operation_log` solo tiene la columna `created_at` (sin `updated_at`). El `save()` predeterminado de Eloquent intentará escribir `updated_at`, lo que provoca un error SQL.
