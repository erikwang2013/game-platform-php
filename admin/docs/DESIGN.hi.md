...
> विस्तृत Mermaid आर्किटेक्चर आरेखों के लिए [ARCHITECTURE.md](ARCHITECTURE.hi.md) देखें (GitHub/GitLab/VS Code में स्वचालित रेंडर)।

## 1. सिस्टम आर्किटेक्चर

> **सुविधा सूची**: प्रमाणीकरण(login/register/refresh/logout + खाता लॉक + सत्र सीमा) | डैशबोर्ड(Redis कैश) | उपयोगकर्ता CRUD+बैच+आयात | भूमिका अनुमतियाँ(RBAC) | सिस्टम कॉन्फ़िगरेशन | ऑपरेशन ऑडिट(8 प्लेटफ़ॉर्म स्रोत) | फ़ाइल(अपलोड+निर्यात+मास्किंग) | सुरक्षा(18 परत सुरक्षा) | संचालन(health/metrics/docs/Docker/CI)
...
- विशेषता: **गैर-ऑटो-इन्क्रीमेंट**, Snowflake एल्गोरिदम से एप्लिकेशन परत पर उत्पन्न
- लाभ: वैश्विक अद्वितीय, वितरित-अनुकूल, ट्रेंड-बढ़ता इंडेक्स-अनुकूल, व्यावसायिक मात्रा उजागर नहीं
...
संस्करण URL पथ प्रीफ़िक्स में होता है (डिफ़ॉल्ट `v1`), अनुरोध हेडर का उपयोग नहीं होता:

| तंत्र | विवरण |
|------|------|
| डिफ़ॉल्ट संस्करण | डिफ़ॉल्ट `v1`, रूट-ग्रुप प्रीफ़िक्स से निर्धारित |
| रूटिंग | रूट-ग्रुप प्रीफ़िक्स `/api/v1`, `/admin/v1` संस्करण को कंट्रोलर नेमस्पेस से मैप करते हैं; `v()` सहायक फ़ंक्शन संस्करण के अनुसार कंट्रोलर क्लास हल करता है |
| निर्देशिका | कंट्रोलर संस्करण के अनुसार: `app/api/{version}/controller/` |

विस्तार उदाहरण — v2 API जोड़ना:
1. `app/api/v2/controller/AuthController.php` बनाएं
2. `/api/v2` रूट ग्रुप रजिस्टर कर कंट्रोलर बांधें
...
सीमा से अधिक पर 429 लौटता है, प्रतिक्रिया हेडर में X-RateLimit-Limit / Remaining / Reset / Retry-After शामिल।

### 4.4 समान प्रतिक्रिया
...
| 400 | पैरामीटर त्रुटि | अनुरोध प्रारूप सही नहीं |
| 401 | प्रमाणित नहीं | Token अनुपस्थित/समाप्त/अमान्य |
| 403 | कोई अनुमति नहीं | उपयोगकर्ता भूमिका में आवश्यक अनुमति नहीं |
| 404 | मौजूद नहीं | संसाधन नहीं मिला |
| 422 | सत्यापन विफल | फ़ॉर्म पैरामीटर नियमों के अनुसार नहीं / पासवर्ड पुष्टि विफल |
...
### 4.7 संवेदनशील ऑपरेशन की द्वितीय पुष्टि

उपयोगकर्ता, भूमिका, अनुमति हटाने जैसे संवेदनशील ऑपरेशनों में अनुरोध निकाय में वर्तमान उपयोगकर्ता का पासवर्ड भेजकर पहचान पुनः सत्यापन आवश्यक:
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | /admin/v1/payment/method/list | सूची (sort आरोही) |
| POST | /admin/v1/payment/method/toggle | सक्षम/अक्षम करें |
| POST | /admin/v1/payment/method/create | बनाएं |
| PUT | /admin/v1/payment/method/{hashid} | अपडेट (केवल भेजे गए फ़ील्ड) |
| DELETE | /admin/v1/payment/method/{hashid} | हटाएं (pending ऑर्डर होने पर 422) |
...
विशेषताएँ: साइडबार फोल्डेबल, Material 3 दोहरी थीम, उच्च-घनत्व डेटा तालिका, पॉपअप Dialog, माउस होवर इंटरैक्शन

### 5.2 HarmonyOS मोबाइल
...
| DashboardPage | `pages/DashboardPage` | स्टैट कार्ड + हालिया ऑपरेशन |
| UserListPage | `pages/UserListPage` | उपयोगकर्ता सूची, खोज + पुल-डाउन रीफ़्रेश + अप-स्वाइप लोड |
| UserDetailPage | `pages/UserDetailPage` | नया/संपादन/देखना/हटाना (AlertDialog पुष्टि) |
| ProfilePage | `pages/ProfilePage` | प्रोफ़ाइल, लॉगआउट (AlertDialog पुष्टि) |
...
| परत | उपाय |
|------|------|
| विधि सीमा | SecurityFilter HTTP मेथड व्हाइटलिस्ट, केवल GET/POST/PUT/DELETE/OPTIONS/HEAD, गैर-मानक विधि 405 |
| आक्रमण अवरोध | SecurityFilter मिडलवेयर, XSS/SQL इंजेक्शन/पाथ ट्रैवर्सल/कमांड इंजेक्शन/CSRF पहचान और अवरोध |
| मानव सत्यापन | क्लिक कैप्चा (Click Captcha), लॉगिन/पंजीकरण पर अनिवार्य |
| खाता लॉक | लगातार 5 असफल लॉगिन पर खाता 15 मिनट लॉक, लॉक अवधि में 429 |
| सत्र सीमा | एक उपयोगकर्ता के अधिकतम 3 समवर्ती Token, अधिक होने पर सबसे पुराना Token स्वचालित ब्लैकलिस्ट |
| दर सीमा | RateLimit मिडलवेयर, Redis स्लाइडिंग विंडो, Lua एटॉमिक |
| CSP | Content-Security-Policy हेडर संसाधन स्रोत सीमित करता है, XSS और डेटा इंजेक्शन रोकता है |
| ऑपरेशन पुष्टि | डिलीट जैसे संवेदनशील ऑपरेशनों में वर्तमान उपयोगकर्ता पासवर्ड की द्वितीय पुष्टि आवश्यक |
| ट्रांसमिशन | HTTPS + JWT Bearer Token |
| इंटरफ़ेस ID | Hashids एन्क्रिप्शन, बाह्य रूप से वास्तविक ID अपरिवर्तनीय |
| अनुरोध निकाय | AES-256-CBC संवेदनशील फ़ील्ड एन्क्रिप्शन |
| डेटाबेस | BIGINT प्राथमिक कुंजी (ऑटो-इन्क्रीमेंट मात्रा उजागर नहीं) |
...
| ऑडिट | OperationLog सभी ऑपरेशन रिकॉर्ड करता है (source स्रोत स्वचालित पहचान सहित) |
...
|------|------|------|
| सूची प्रदर्शन | phone | मास्क: 138****1234 |
| सूची प्रदर्शन | email | मास्क: a***@example.com |
| विवरण देखना | phone/email | डिक्रिप्ट करने वाला इंटरफ़ेस आवश्यक |
| Excel निर्यात | phone/email | मास्क करके निर्यात |
| PDF निर्यात | सभी फ़ील्ड | मास्क + न हटाने योग्य कॉपीराइट वॉटरमार्क |
| स्टोरेज | phone/email/id_card | encryptable से साइफरटेक्स्ट एन्क्रिप्शन |
...
प्रोजेक्ट रूट की `docker-compose.yml` उपरोक्त टोपोलॉजी की सभी सेवाओं का ऑर्केस्ट्रेशन करती है:
...
शुरू करने से पहले `docker-compose.yml` में `JWT_SECRET`, `HASHIDS_SALT`, `ENCRYPTION_KEY` जैसी कुंजियाँ यादृच्छिक स्ट्रिंग में बदलें।
...
`database/backup/backup.sh` — mysqldump + gzip बैकअप, 30 दिन पुराना बैकअप स्वचालित सफाई।
