...
flowchart LR
    ...
    subgraph store["2.Speichern"]
        s1["MySQL-Tabelle game_*<br/>id BIGINT UNSIGNED NOT NULL"]
        s2["Sensible Felder encryptable cast<br/>AES-128-ECB-verschlüsselte Speicherung"]
        g3 --> s1 --> s2
    end

    subgraph transfer["3.Übertragen"]
        t1["HashidsService::encode(bigint)"]
        t2["hashid-Zeichenkette<br/>z. B. aB3xK9mW2pQ7rT5v"]
        s1 --> t1 --> t2
    end

    subgraph reverse["4.Rückwärtsdekodierung"]
