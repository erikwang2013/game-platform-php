# الإدارة الخلفية المفتوحة — وثيقة التصميم
<!-- lang-nav -->
...
| وحدات التحكم | 45 وحدة: Dashboard/User/Role/Permission/Config/Log/Profile/Export/Import/Upload/Health/Docs/Metrics/Analytics/Game/Payment/Withdraw... (الإدارة) + Captcha/Auth (API v1) | التحقق من معاملات الطلب، استدعاء منطق الأعمال، تنسيق الاستجابة |
| خدمات الأعمال | `packages/platform-common/src/service/` | تحليل البيانات: GameDashboardService (نظرة عامة/ترتيب/اتجاه)، DepositLogService (إيراد/تحويل)، ProbabilityService (احتمال مشترك/شرطي، منشئ SQL)؛ عند تعطل DB تُرجع بيانات فارغة بدلاً من خطأ |
| نماذج البيانات | `app/model/` | تعيين ORM، علاقات الارتباط، تشفير/فك تشفير الحقول |
| الأدوات العامة | `packages/platform-common/src/` | خدمات Hashids وSnowflake وEncryption |
...
العميل                                الخادم
  ...
  │                                    │
  │  ② النقر على مواضع النص في الصورة    │
  │                                    │
...
│ 👥 المستخدمون │  └──────────────┘ └──────────┘ │
│ 🔒 الأدوار │  ┌──────┐ ┌────────────────┐     │
│ ⚙ الإعدادات │  │مخطط دائري│ │ أحدث السجلات│   │
│ 📋 السجلات │  └──────┘ └────────────────┘     │
└──────────┴─────────────────────────────────────┘
```
...
| `app` | بناء محلي عبر `Dockerfile` | 8789 | PHP 8.3 + OPcache + webman |
...
يكشف نقطة النهاية `GET /metrics` (عبر `MetricsController`) 18 مقياس gauge بصيغة Prometheus text: المستخدمون النشطون/الإجمالي، حالة اتصال قاعدة البيانات/Redis/ES، مقاييس الذاكرة/CPU/العمليات، السحوبات المعلقة وفروقات التسوية.
