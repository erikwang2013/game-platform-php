...
class UserListPage extends GetView<UserController> {
      ...
      children: [
        ...
        Row(
          children: [
            ...
            ChoiceChip(label: Text("${AppTranslations.t('app.enabled')}"), selected: ctrl.statusFilter.value == 1, onSelected: (_) => ctrl.filterByStatus(1)),
            const SizedBox(width: 4),
            ChoiceChip(label: Text("${AppTranslations.t('app.disabled')}"), selected: ctrl.statusFilter.value == 0, onSelected: (_) => ctrl.filterByStatus(0)),
          ],
        ),
        const SizedBox(height: 12),
        // Table
            ...
            return SingleChildScrollView(
              child: DataTable(
                columns: [
                  ...
                  DataColumn(label: Text('${AppTranslations.t('user.real_name')}')),
                  DataColumn(label: Text('${AppTranslations.t('user.phone')}')),
                  DataColumn(label: Text('${AppTranslations.t('user.email')}')),
                  DataColumn(label: Text('${AppTranslations.t('user.status')}')),
                  DataColumn(label: Text('${AppTranslations.t('user.last_login')}')),
                  DataColumn(label: Text('${AppTranslations.t('user.actions')}')),
                ],
  ...
  void _confirmBatchDelete(BuildContext context, UserController ctrl) {
    ...
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('${AppTranslations.t('user.confirm_batch_delete')}'),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
