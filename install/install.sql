...
CREATE TABLE IF NOT EXISTS `game_exchange_record` (
    ...
    `platform_amount` DECIMAL(18,4) NOT NULL COMMENT '平台币数量（in=支出的平台币 / out=扣点差后到账的平台币净额）',
    `game_amount` DECIMAL(18,4) NOT NULL COMMENT '游戏币数量（in=扣点差后到账的游戏币净额 / out=卖出的游戏币数）',
    `rate` DECIMAL(18,8) NOT NULL COMMENT '成交汇率',
    `spread_fee` DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '点差费用（in: 游戏币 / out: 平台币）',
...
CREATE TABLE IF NOT EXISTS `game_language` (
    ...
    `country_code` VARCHAR(10) NOT NULL DEFAULT '' COMMENT '关联国家(ISO 3166-1 alpha-2，空=通用)',
    `status` TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '状态: 0=禁用 1=启用',
    `sort` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '排序',
...
CREATE TABLE IF NOT EXISTS `game_country_config` (
    ...
    `lang_prefix` VARCHAR(10) NOT NULL DEFAULT '' COMMENT '语言前缀->国家映射(zh/en/ja/...)，CountryConfig::fromLang() 查表用，空=无映射',
    `currency` VARCHAR(10) NOT NULL DEFAULT 'USD' COMMENT '默认币种',
    `payment_methods` TEXT COMMENT '可用支付方式(JSON规则: 键=网关provider，值=min/max/fee_percent/enabled)',
    `withdraw_methods` TEXT COMMENT '可用提现方式(JSON规则: 键=paypal/bank/crypto，值=min/max/fee_percent/enabled)',
    `min_deposit` DECIMAL(18,4) NOT NULL DEFAULT 1.0000 COMMENT '最低充值额',
    `max_deposit` DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '最高单笔充值额(0=不限)',
    `daily_deposit_limit` DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '单日充值限额(0=不限)',
    `withdraw_fee_percent` DECIMAL(10,4) NOT NULL DEFAULT 0.0000 COMMENT '提现费率%(对账参考值，真实费率以网关侧为准)',
    `withdraw_min` DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '最低提现额(0=不限)',
    `settlement_days` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '结算周期 T+N(0=实时)',
    `status` TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '0=禁用 1=启用',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
(30000000000000002, 'zh-CN', 'Chinese (Simplified)', '简体中文', 'cn', 'CN', 1, 2),
(30000000000000003, 'ja-JP', 'Japanese', '日本語', 'jp', 'JP', 1, 3),
(30000000000000004, 'ko-KR', 'Korean', '한국어', 'kr', 'KR', 1, 4),
(30000000000000005, 'pt-BR', 'Portuguese (Brazil)', 'Português (Brasil)', 'br', 'BR', 1, 5),
(30000000000000006, 'hi-IN', 'Hindi', 'हिन्दी', 'in', 'IN', 1, 6),
(30000000000000007, 'de-DE', 'German', 'Deutsch', 'de', 'DE', 1, 7),
(30000000000000008, 'en-SG', 'English (Singapore)', 'English (Singapore)', 'sg', 'SG', 1, 8),
(30000000000000009, 'ms-MY', 'Malay', 'Bahasa Melayu', 'my', 'MY', 1, 9),
(30000000000000010, 'th-TH', 'Thai', 'ไทย', 'th', 'TH', 1, 10),
(30000000000000011, 'id-ID', 'Indonesian', 'Bahasa Indonesia', 'id', 'ID', 1, 11),
(30000000000000012, 'vi-VN', 'Vietnamese', 'Tiếng Việt', 'vn', 'VN', 1, 12),
(30000000000000013, 'tl-PH', 'Filipino', 'Filipino', 'ph', 'PH', 1, 13),
(30000000000000014, 'en-ZA', 'English (South Africa)', 'English (South Africa)', 'za', 'ZA', 1, 14),
(30000000000000015, 'en-NG', 'English (Nigeria)', 'English (Nigeria)', 'ng', 'NG', 1, 15),
(30000000000000016, 'es-AR', 'Spanish (Argentina)', 'Español (Argentina)', 'ar', 'AR', 1, 16),
(30000000000000017, 'es-CL', 'Spanish (Chile)', 'Español (Chile)', 'cl', 'CL', 1, 17);
...
(50000000000000069, 'Adyen', 'fiat', 'adyen', '{}', 1, 180, '[]', '', 0.0000, 0.0000),
(50000000000000070, 'GrabPay', 'fiat', 'grabpay', '{"country":"SG"}', 1, 190, '["SG"]', '', 0.0000, 0.0000);
...
-- 默认国家配置
INSERT IGNORE INTO `game_country_config` (`id`, `country_code`, `lang_prefix`, `currency`, `payment_methods`, `withdraw_methods`, `min_deposit`, `max_deposit`, `daily_deposit_limit`, `withdraw_fee_percent`, `withdraw_min`, `settlement_days`, `status`) VALUES
(50000000000000101, 'US', 'en', 'USD', '["stripe","paypal","crypto"]', '["paypal","bank","crypto"]', 1.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
(50000000000000102, 'CN', 'zh', 'CNY', '["crypto","alipay","wechat"]', '["alipay","bank"]', 10.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
(50000000000000103, 'JP', 'ja', 'JPY', '["stripe","paypal"]', '["paypal","bank"]', 100.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
(50000000000000104, 'KR', 'ko', 'KRW', '["stripe","paypal"]', '["paypal","bank"]', 1000.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
(50000000000000105, 'GB', '', 'GBP', '["stripe","paypal"]', '["paypal","bank"]', 1.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
(50000000000000106, 'DE', 'de', 'EUR', '["stripe","paypal"]', '["paypal","bank"]', 1.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
(50000000000000107, 'BR', 'pt', 'BRL', '["stripe"]', '["bank"]', 1.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
(50000000000000108, 'IN', 'hi', 'INR', '["stripe"]', '["bank"]', 50.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0, 1),
