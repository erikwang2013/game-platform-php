...
القدرات المنفذة: JWT+RBAC، القفل التفاؤلي للمحفظة، الشحن (تحقق Stripe/PayPal)، فرق الاستبدال، مراجعة السحب + دفع PayPal، CRUD الألعاب/بوابة Provider (HMAC)/القسائم/VIP/الإنجازات/التذاكر/عمولة الإحالة/2FA/الاجتماعي (أصدقاء/محادثة WS)/البطولات/Webhook/الدفع (FCM/APNs/هواوي)/i18n ثنائية اللغة.
...
1. **fail-closed لاستدعاء الدفع**: القائمة البيضاء للمزود (stripe/paypal فقط) + رفض 500 إلزامي عند غياب المفتاح + رفض إلزامي لأي استثناء PayPal (C1/C2) — ✅ مكتمل (2026-08-18: القائمة البيضاء للمزود + التحقق من انتحال القنوات المتقاطعة + تحقق اختياري من IP المصدر + معاملاتية الإيداع في الاستدعاء)
