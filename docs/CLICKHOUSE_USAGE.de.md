# ClickHouse-Service-Nutzungsleitfaden
<!-- lang-nav -->
...
Alle Dienste liegen in `common/service/` und werden als statische Methoden aufgerufen.
...
Ereignis: `['table' => 'Tabellenname', 'alias' => 'Verknüpfungsfeld', 'where' => [...], 'whereRaw' => '...']`
...
Eingebunden in `GameController::launch()`.
...
Eingebunden in `DepositController::create()` und `PaymentController::callback()`.
...
GameDashboardService::gameRanking(7);           // Rangliste
GameDashboardService::dauTrend(30);             // DAU-Trend
GameDashboardService::hourlyTrend(5, 7);        // Tageszeiten
GameDashboardService::actionDistribution(5, 24); // Verhaltensverteilung
```

## 6. Backend-Dashboard-APIs
