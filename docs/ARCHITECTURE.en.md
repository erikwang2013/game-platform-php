...
flowchart TB
    subgraph "Client Layer"
        ...
        A5["React · Angular<br/>C-end user platform"]
    end

    subgraph "Gateway Layer (Nginx)"
        B1["Reverse proxy + HTTPS<br/>Route dispatch + Gzip<br/>Static file serving"]
    end

    subgraph "Application Layer"
        C1["admin/ webman<br/>Admin panel :8789<br/>AdminAuth → AdminPermission → OperationLog"]
        C2["service/ webman<br/>C-end business :8792<br/>UserAuth → [ProviderAuth]"]
    end

    subgraph "Service Layer (new)"
        D0["GameProvider abstraction layer<br/>SelfProvider / ThirdPartyProvider<br/>HMAC-SHA256 signature<br/>Transactional consistency guarantee"]
        D1["EventBus<br/>Redis Pub/Sub<br/>Async event dispatch<br/>Achievements/notifications/audit decoupling"]
        D2["VIP engine<br/>EXP accumulation→auto level-up<br/>Exchange discount/withdrawal fee reduction<br/>Exchange-rate bonus"]
        D3["Achievement engine<br/>12 built-in achievements<br/>Progress tracking<br/>Event-driven detection"]
        D4["Feature flags<br/>FeatureFlag<br/>Zero-dependency dynamic config"]
    end

    subgraph "Data Layer"
        E1[("MySQL 8.0<br/>Primary storage<br/>78 tables")]
        E2[("Redis<br/>Session/cache/rate limiting<br/>EventBus/heartbeat")]
        E3[("Elasticsearch<br/>Full-text search")]
        E4[("ClickHouse<br/>OLAP analysis<br/>Probability calculation")]
    end

    subgraph "External Integrations"
        F1["Third-party games<br/>Provider API<br/>balance/bet/settle/refund"]
        F2["Push channels<br/>FCM / APNs<br/>Huawei Push"]
        F3["OAuth (7 platforms)<br/>Google/Facebook/Apple<br/>X(Twitter)/Microsoft<br/>LinkedIn/GitHub"]
    end
...
### 2.2 service/ — C-end Business Service
...
### 2.3 Provider Layer — Game Integration Abstraction
...
### 2.5 Stability — Circuit Breaker / Retry / Degradation
...
Degradation switch `feature.provider_mock` (FeatureFlag / PlatformConfig, short-circuits real network calls when `on`):

| Entry point | Behavior when mock=on |
|--------|-------------|
| `PushService::send` | Returns immediately, no push sent |
| `PayoutService::execute` | Returns `mock-{order_no}` batch and marks order completed |
| `ThirdPartyProvider::request` | Returns `['success' => true]` |

All real network calls are wrapped in `Retry::run → CircuitBreaker::call` (Push FCM/APNs/HarmonyOS, PayPal payouts, third-party Provider requests).

## 3. Middleware Execution Chains
...
### service/ (C-end Business Service)
...
### 4.4 Game Provider Interaction Flow
...
flowchart TB
    ...
    subgraph "Web Server (Nginx)"
        NGX["Reverse proxy :443 HTTPS<br/>Static file serving<br/>gzip + CSP + HSTS<br/>limit_req rate limiting"]
    end
    ...
    subgraph "Data Layer"
        MYSQL["MySQL 8.0 primary-replica replication"]
        REDIS["Redis 7.x Sentinel mode<br/>EventBus Pub/Sub"]
...
| leaderboard-ws | 8790 | WebSocket real-time leaderboard |
| chat-ws | 8791 | WebSocket private messages/chat |
| MySQL | 3306 | Main database |
| Redis | 6379 | Cache/rate limit/WebSocket/EventBus |
| ClickHouse | 8123 | OLAP HTTP interface |
| Elasticsearch | 9200 | Full-text search |

## 9. API Documentation

Interactive API docs are auto-generated from controller annotations via `erikwang2013/apidoc-php`:

| Doc | URL | Controllers | Endpoints |
|------|------|--------|------|
| Admin backend | :8789/apidoc/ | 45 | 154 |
| C-end business | :8792/apidoc/ | 34 | 107 |

## 10. Database Table List

### Basic Version (12 tables) + admin (7 tables)
...
### Ecosystem Expansion (14 tables) ← New
...
Based on the `feature.*` namespace of `game_platform_config`, zero extra dependencies:

| Flag | Default | Feature |
|------|------|------|
| feature.tournament | off | Tournament system |
| feature.chat | off | WebSocket private messaging |
| feature.vip | off | VIP loyalty |
| feature.achievements | off | Achievement badges |
