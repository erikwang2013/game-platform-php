...
> Hinweis: Eine Änderung von `ADMIN_PORT` / `SERVICE_PORT` aktualisiert nicht automatisch `APP_URL` in `admin/.env` und `SITE_URL` in `service/.env`; die externen Zugriffsadressen müssen ebenfalls angepasst werden.
