...
Vérification : `clickhouse-client -q "SELECT version()"` ou `curl 'http://localhost:8123/?query=SELECT+1'`

## 2. Variables d'environnement

Modifier `service/.env` :
...
`composer.json` inclut déjà `erikwang2013/clickhouse-php: ^1.0`, configuré dans `config/plugin/erikwang2013/clickhouse-php/app.php`.
