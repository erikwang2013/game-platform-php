...
- 42 `game_`-prefixed data tables (CREATE TABLE IF NOT EXISTS)
- 13 INSERT IGNORE seed data blocks
- The `source` field of `game_operation_log` is merged into the CREATE TABLE statement (no ALTER TABLE needed)
...
| Table | Handling |
|------|---------|
| `game_admin_role` | INSERT IGNORE (fixed IDs) |
| `game_admin_permission` | INSERT IGNORE (fixed IDs) - 4 times |
| `game_admin_role_permission` | WHERE NOT EXISTS subquery |
| `game-platform_config` | INSERT IGNORE (fixed IDs) - 2 times |
| `game_language` | INSERT IGNORE (fixed IDs) |
| `game_translation` | INSERT IGNORE (fixed IDs) |
| `game_risk_rule` | INSERT IGNORE (fixed IDs) |
| `game_withdraw_limit` | INSERT IGNORE (fixed IDs) |
| `game_game_category` | INSERT IGNORE (fixed IDs) |
| `game_country_config` | INSERT IGNORE (fixed IDs) |
...
|---|------|--------|------|
| 1 | `game_admin_role_permission` INSERT missing the `WHERE NOT EXISTS` guard (inconsistent with original migration) | High | Fixed |
...
- After removing hardcoded `game_` prefixes from models, actual table names are still uniformly generated by `prefix=game_` in `config/database.php`, consistent with the `game_*` tables created by install.sql; no install SQL changes needed
