...
すべてのサービスは `common/service/` にあり、静的メソッドで呼び出す。
...
イベント: `['table' => 'テーブル名', 'alias' => '関連フィールド', 'where' => [...], 'whereRaw' => '...']`
...
`DepositController::create()` と `PaymentController::callback()` に接続済み。
