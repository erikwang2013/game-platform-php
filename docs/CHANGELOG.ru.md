...
- Отчёты по данным: админка `/admin/report/summary|daily|export` (сводный/ежедневный/CSV-экспорт, кэш Redis 5 мин, период ≤90 дней).
- Статистика платформы (C-сторона): `GET /api/platform/stats` (всего игр/пользователей/игр сегодня/активных за 7 дней), подключена к главной странице.
- Flutter админки: карточки дашборда подключены к реальным данным, добавлена страница отчётов ReportsPage (/reports).
- Синхронизация документации: FEATURES/VERSIONS/API дополнены записями об отчётах и статистике на 13 языках, обновлён блок статистики на схеме возможностей.
