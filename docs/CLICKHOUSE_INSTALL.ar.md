...
التحقق: `clickhouse-client -q "SELECT version()"` أو `curl 'http://localhost:8123/?query=SELECT+1'`
...
يتضمن `composer.json` بالفعل `erikwang2013/clickhouse-php: ^1.0`، والتهيئة موجودة في `config/plugin/erikwang2013/clickhouse-php/app.php`.
