...
| サービス | ポート | 説明 |
|------|------|------|
| admin/ | 8787 | 管理バックエンド API |
| service/ | 8788 | C端業務 API |
| leaderboard-ws | 8789 | WebSocket リアルタイムランキング |
...
`hg/apidoc` でコントローラーアノテーションからインタラクティブな API ドキュメントを自動生成:

| ドキュメント | アドレス | コントローラー | エンドポイント |
|------|------|--------|------|
| 管理バックエンド | :8787/apidoc/ | 28 | ~85 |
| C端業務 | :8788/apidoc/ | 25 | ~65 |
...
`game-platform_config` の `feature.*` 名前空間に基づき、追加依存ゼロ:
