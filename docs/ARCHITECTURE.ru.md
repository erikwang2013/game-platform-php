...
|------|------|------|
| admin/ | 8787 | API админ-панели |
| service/ | 8788 | API C-стороннего бизнеса |
| leaderboard-ws | 8789 | WebSocket-рейтинг в реальном времени |
...
Интерактивная документация API генерируется автоматически через `hg/apidoc` по аннотациям контроллеров:

| Документация | Адрес | Контроллеры | Эндпоинты |
|------|------|--------|------|
| Админ-панель | :8787/apidoc/ | 28 | ~85 |
| C-сторонний бизнес | :8788/apidoc/ | 25 | ~65 |

## 10. Список таблиц базы данных

### Базовая версия (14 таблиц) + admin (7 таблиц)
...
### Экосистемное расширение (10 таблиц) ← добавлено
...
Пространство имён `feature.*` на базе `game-platform_config`, без дополнительных зависимостей:
