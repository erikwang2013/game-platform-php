...
flowchart TB
    subgraph "Клиентский слой"
        ...
        A4["React · Angular<br/>Админ-панель"]
        A5["React · Angular<br/>Пользовательская платформа C-стороны"]
    end

    subgraph "Слой шлюза (Nginx)"
        B1["Обратный прокси + HTTPS<br/>Маршрутизация + Gzip<br/>Раздача статики"]
    end

    subgraph "Слой приложения"
        C1["admin/ webman<br/>Админ-панель :8789<br/>AdminAuth → AdminPermission → OperationLog"]
        C2["service/ webman<br/>Бизнес C-стороны :8792<br/>UserAuth → [ProviderAuth]"]
    end

    subgraph "Слой сервисов (новый)"
        D0["Слой абстракции GameProvider<br/>SelfProvider / ThirdPartyProvider<br/>Подпись HMAC-SHA256<br/>Гарантия согласованности транзакций"]
        D1["EventBus<br/>Redis Pub/Sub<br/>Асинхронная доставка событий<br/>Развязка достижений/уведомлений/аудита"]
        D2["VIP-движок<br/>Накопление опыта→автоповышение<br/>Скидка на обмен/уменьшение комиссии вывода<br/>Надбавка к курсу"]
        D3["Движок достижений<br/>12 встроенных достижений<br/>Отслеживание прогресса<br/>Событийная проверка"]
        D4["Переключатель функций<br/>FeatureFlag<br/>Динамическая конфигурация без зависимостей"]
    end

    subgraph "Слой хранения"
        E1[("MySQL 8.0<br/>Основное хранилище<br/>78 таблиц")]
        E2[("Redis<br/>Session/кэш/лимиты<br/>EventBus/heartbeat")]
        E3[("Elasticsearch<br/>Полнотекстовый поиск")]
        E4[("ClickHouse<br/>Анализ OLAP<br/>Расчёт вероятностей")]
    end

    subgraph "Внешние интеграции"
        F1["Сторонние игры<br/>Provider API<br/>Баланс/ставка/settle/возврат"]
        F2["Канал пушей<br/>FCM / APNs<br/>Huawei Push"]
        F3["OAuth (7 платформ)<br/>Google/Facebook/Apple<br/>X(Twitter)/Microsoft<br/>LinkedIn/GitHub"]
    end
...
### 2.2 service/ — C-сторонний бизнес
...
### 2.3 Provider-слой — абстракция подключения игр
...
### 2.5 Обеспечение стабильности — предохранитель / повтор / деградация
...
Переключатель деградации `feature.provider_mock` (FeatureFlag / PlatformConfig, при `on` пропускает реальные сетевые вызовы):

| Точка входа | Поведение при mock=on |
|--------|-------------|
| `PushService::send` | Немедленный возврат, отправка не выполняется |
| `PayoutService::execute` | Возвращает пакет `mock-{order_no}` и помечает заказ completed |
| `ThirdPartyProvider::request` | Возвращает `['success' => true]` |

Все реальные сетевые вызовы обёрнуты в `Retry::run → CircuitBreaker::call` (Push FCM/APNs/HarmonyOS, выплаты PayPal, запросы сторонних Provider).

## 3. Цепочки выполнения промежуточных слоёв
...
### 4.4 Взаимодействие с игровым Provider
...
### 4.5 Поток повышения VIP-уровня
...
flowchart TB
    ...
    subgraph "Веб-сервер (Nginx)"
        NGX["Обратный прокси :443 HTTPS<br/>Раздача статики<br/>gzip + CSP + HSTS<br/>Ограничение limit_req"]
    end
    ...
    subgraph "Слой данных"
        MYSQL["MySQL 8.0 репликация мастер-реплика"]
        REDIS["Redis 7.x режим sentinel<br/>EventBus Pub/Sub"]
...
|------|------|------|
| admin/ | 8789 | API админ-панели |
| service/ | 8792 | API C-стороннего бизнеса |
| leaderboard-ws | 8790 | WebSocket-рейтинг в реальном времени |
| chat-ws | 8791 | WebSocket-личные сообщения/чат |
| MySQL | 3306 | Основная база данных |
| Redis | 6379 | Кэш/лимиты/WebSocket/EventBus |
| ClickHouse | 8123 | HTTP-интерфейс OLAP |
| Elasticsearch | 9200 | Полнотекстовый поиск |

## 9. Документация API

Интерактивная документация API генерируется автоматически через `erikwang2013/apidoc-php` по аннотациям контроллеров:

| Документация | Адрес | Контроллеры | Эндпоинты |
|------|------|--------|------|
| Админ-панель | :8789/apidoc/ | 45 | 154 |
| C-сторонний бизнес | :8792/apidoc/ | 34 | 107 |

## 10. Список таблиц базы данных

### Базовая версия (12 таблиц) + admin (7 таблиц)
...
### Экосистемное расширение (14 таблиц) ← добавлено
...
## 11. Функциональные переключатели

Пространство имён `feature.*` на базе `game_platform_config`, без дополнительных зависимостей:

| Переключатель | По умолчанию | Функция |
|------|------|------|
| feature.tournament | off | Система турниров |
| feature.chat | off | WebSocket-личные сообщения |
| feature.vip | off | VIP-программа лояльности |
| feature.achievements | off | Бейджи достижений |
