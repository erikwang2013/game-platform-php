# Руководство по использованию сервисов ClickHouse
<!-- lang-nav -->
...
Все сервисы находятся в `common/service/`, вызовы через статические методы.
...
Событие: `['table' => 'имя таблицы', 'alias' => 'связующее поле', 'where' => [...], 'whereRaw' => '...']`
...
Подключён в `GameController::launch()`.
...
Подключён в `DepositController::create()` и `PaymentController::callback()`.
