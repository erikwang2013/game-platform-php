...
| H3 | common/ 与 model/ 双份复制且已漂移（DepositLogService 两份内容不同、User.php 不一致），单点修复变双份工作 | admin/common vs service/common |
| H4 | HarmonyOS C端 `apps/harmonyos/` 为空目录，0 页 vs VERSIONS.md 声称 5 页 | apps/harmonyos/ |
...
- 文档与实际不符：install.sql 43 表 vs 文档 52；docker-compose 7 服务 vs FEATURES.md 8；"共享 Model 34" 不实（实为双份 52）；无 CHANGELOG，v1.1 改动只散见提交信息
...
16. **HarmonyOS C端** 从零实现 5 页（登录/大厅/详情/钱包/个人）（H4）
...
19. **Prometheus 业务指标**（事件投递/消费率、队列深度）+ 灰度 AB 分流中间件（复用 FeatureFlag）
20. **WebSocket 数据链路闭环**：排行榜/聊天持久化确认
21. **文档对齐**：表数/服务数/共享层描述修正、API 文档与实现对齐、补 CHANGELOG
